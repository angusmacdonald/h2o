...
public interface IWorker {
     ...
     */
    public boolean startH2OInstance(final String descriptorFileLocation);

    /**
     * Gracefully shutdown the H2O instance that this worker is running (by sending it a shutdown request).
     * @return true if the H2O was successfully stopped. In cases where it couldn't be stopped or wasn't even running, false is returned.
     */
    public boolean stopH2OInstance();

    /**
     * Abruptly terminate the H2O instance that is running locally (by issuing a kill command on the process).
     * @return true if the H2O was successfully stopped. In cases where it couldn't be stopped or wasn't even running, false is returned.
     */
    public boolean terminateH2OInstance();
     ...
     */
    public boolean isH2OInstanceRunning();
     ...
     */
    public boolean startWorkload(IWorkload workload);
     ...
     */
    public boolean stopWorkload(IWorkload workload);
     ...
     */
    public boolean isWorkloadRunning(IWorkload workload);
}
