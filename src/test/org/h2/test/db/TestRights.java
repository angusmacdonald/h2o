...
public class TestRights extends TestBase {
    ...
    private void testGetTables() throws SQLException, IOException {

        deleteDb("rights");
        final Connection conn = getConnection("rights");
        stat = conn.createStatement();

        stat.execute("CREATE USER IF NOT EXISTS TEST PASSWORD 'TEST'");
        stat.execute("CREATE TABLE TEST(ID INT)");
        stat.execute("GRANT ALL ON TEST TO TEST");
        final Connection conn2 = getConnection("rights", "TEST", getPassword("TEST"));
        final DatabaseMetaData meta = conn2.getMetaData();
        meta.getTables(null, null, "%", new String[]{"TABLE", "VIEW", "SEQUENCE"});
        conn2.close();
        conn.close();
    }

    private void testDropTempTables() throws SQLException, IOException {
        ...
        stat.execute("CREATE TABLE TEST(ID INT)");
        final Connection conn2 = getConnection("rights", "READER", getPassword("READER"));
    ...
    private void testSchemaRenameUser() throws SQLException, IOException {
    ...
    private void testAccessRights() throws SQLException, IOException {
    ...
    private void testTableType(final Connection conn, final String type) throws SQLException {

        executeSuccess("SET DEFAULT_TABLE_TYPE " + type);
        executeSuccess("CREATE TABLE TEST(ID INT)");
        final ResultSet rs = conn.createStatement().executeQuery("SELECT STORAGE_TYPE FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='TEST'");
        rs.next();
        assertEquals(rs.getString(1), type);
        executeSuccess("DROP TABLE TEST");
    }

    private void executeError(final String sql) throws SQLException {
    ...
    private void executeSuccess(final String sql) throws SQLException {
