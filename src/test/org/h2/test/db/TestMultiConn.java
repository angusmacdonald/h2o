...
public class TestMultiConn extends TestBase implements DatabaseEventListener {
    ...
    private void testThreeThreads() throws Exception {
        ...
        t2.join(1000);
        t1.join(1000);
        final ResultSet rs = s1.executeQuery("SELECT * FROM TEST1 ORDER BY ID");
        rs.next();
    ...
    private void testConcurrentOpen() throws Exception {
        ...
        deleteDb("multiConn");
        final Connection conn = getConnection("multiConn");
        ...
        final Runnable r = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    final Connection c1 = getConnection("multiConn;DATABASE_EVENT_LISTENER='" + listener + "';file_lock=socket");
                    c1.close();
                }
                catch (final Exception e) {
        ...
        thread.start();
        Thread.sleep(10);
        final Connection c2 = getConnection("multiConn;file_lock=socket");
        c2.close();
        thread.join();
    }
    ...
    private void testCommitRollback() throws SQLException, IOException {

        deleteDb("multiConn");
        final Connection c1 = getConnection("multiConn");
        final Connection c2 = getConnection("multiConn");
