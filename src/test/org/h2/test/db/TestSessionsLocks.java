...
public class TestSessionsLocks extends TestBase {
    ...
    private void testLocks() throws SQLException, IOException {

        deleteDb("sessionsLocks");
        final Connection conn = getConnection("sessionsLocks;MULTI_THREADED=1");
        final Statement stat = conn.createStatement();
        ResultSet rs;
        rs = stat.executeQuery("select * from information_schema.locks order by session_id");
        assertFalse(rs.next());
        final Connection conn2 = getConnection("sessionsLocks");
    ...
    private void testCancelStatement() throws Exception {
        ...
        assertTrue(rs.next());
        final int otherId = rs.getInt("ID");
