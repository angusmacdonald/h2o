...
/**
 * This test is similar to the TPC-B test of the Transaction Processing Council (TPC). Multiple threads are used (one thread per
 * connection). Referential integrity is not implemented.
 * <p>
...
public class BenchB implements Bench, Runnable {
	...
	private BenchB(BenchB master, int seed) throws SQLException {
		...
		updateAccount = conn.prepareStatement("UPDATE ACCOUNTS SET ABALANCE=ABALANCE+? WHERE AID=?");
		selectAccount = conn.prepareStatement("SELECT ABALANCE FROM ACCOUNTS WHERE AID=?");
		updateTeller = conn.prepareStatement("UPDATE TELLERS SET TBALANCE=TBALANCE+? WHERE TID=?");
		updateBranch = conn.prepareStatement("UPDATE BRANCHES SET BBALANCE=BBALANCE+? WHERE BID=?");
		insertHistory = conn.prepareStatement("INSERT INTO HISTORY(TID, BID, AID, DELTA) VALUES(?, ?, ?, ?)");
	}
	...
	public void init(Database db, int size) throws SQLException {
		...
		prep = db.prepare("INSERT INTO BRANCHES(BID, BBALANCE) VALUES(?, 0)");
