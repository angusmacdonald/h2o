...
/**
 * This test is similar to the TPC-C test of the Transaction Processing Council (TPC). Only one connection and one thread is used.
...
public class BenchC implements Bench {
	...
	private static final String[] TABLES = new String[] { "WAREHOUSE", "DISTRICT", "CUSTOMER", "HISTORY", "ORDERS", "NEW_ORDER", "ITEM",
	...
	private void loadItem() throws SQLException {
		...
		PreparedStatement prep = db.prepare("INSERT INTO ITEM(I_ID, I_IM_ID, I_NAME, I_PRICE, I_DATA) " + "VALUES(?, ?, ?, ?, ?)");
	...
	private void loadCustomerSub(int dId, int wId) throws SQLException {
		...
		PreparedStatement prepCustomer = db.prepare("INSERT INTO CUSTOMER(C_ID, C_D_ID, C_W_ID, " + "C_FIRST, C_MIDDLE, C_LAST, "
				+ "C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, " + "C_PHONE, C_SINCE, C_CREDIT, "
				+ "C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_DATA, " + "C_YTD_PAYMENT, C_PAYMENT_CNT, C_DELIVERY_CNT) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	private void loadOrderSub(int dId, int wId) throws SQLException {
		...
		PreparedStatement prepNewOrder = db.prepare("INSERT INTO NEW_ORDER (NO_O_ID, NO_D_ID, NO_W_ID) " + "VALUES (?, ?, ?)");
	...
	private void loadStock(int wId) throws SQLException {
		...
		PreparedStatement prep = db.prepare("INSERT INTO STOCK(S_I_ID, S_W_ID, S_QUANTITY, "
				...
				+ "S_DATA, S_YTD, S_ORDER_CNT, S_REMOTE_CNT) " + "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		for ( int id = 1; id <= items; id++ ) {
