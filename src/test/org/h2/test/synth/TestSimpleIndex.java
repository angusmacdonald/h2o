...
public class TestSimpleIndex extends TestBase {
    ...
    private void testIndex(final int seed) throws SQLException {

        random.setSeed(seed);
        final String unique = random.nextBoolean() ? "UNIQUE " : "";
        final int len = random.getInt(2) + 1;
        ...
        final String cols = buff.toString();
    ...
    private void testRows() throws SQLException {
        ...
        final String a = randomValue(), b = randomValue(), c = randomValue();
        final String data = a + "/" + b + "/" + c;
        final String sql = "VALUES(" + a + ", " + b + ", " + c + ", '" + data + "')";
        boolean em, ed;
        // if(id==73) {
        // print("halt");
        // }
        ...
        final StringBuilder buff = new StringBuilder("WHERE 1=1");
        final int len = random.getLog(10);
        for (int i = 0; i < len; i++) {
            buff.append(" AND ");
        ...
        final String where = buff.toString();
        final String r1 = getResult("SELECT DATA FROM TEST_M " + where + " ORDER BY DATA");
        final String r2 = getResult("SELECT DATA FROM TEST_D " + where + " ORDER BY DATA");
        final String r3 = getResult("SELECT DATA FROM TEST_MI " + where + " ORDER BY DATA");
        final String r4 = getResult("SELECT DATA FROM TEST_DI " + where + " ORDER BY DATA");
        assertEquals(r1, r2);
        assertEquals(r1, r3);
        assertEquals(r1, r4);
    }

    private String getResult(final String sql) throws SQLException {
