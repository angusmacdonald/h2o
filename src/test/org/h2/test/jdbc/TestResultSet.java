...
public class TestResultSet extends TestBase {
    ...
    private void testAbsolute() throws SQLException {
        ...
        final Statement s2 = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
        final ResultSet rs = s2.executeQuery("SELECT * FROM TEST ORDER BY ID");
        for (int i = 100; i > 0; i--) {
            rs.absolute(i);
    ...
    private void testFetchSize() throws SQLException {
        ...
        if (!config.networked || config.memory) { return; }
        final ResultSet rs = stat.executeQuery("SELECT * FROM SYSTEM_RANGE(1, 100)");
        final int a = stat.getFetchSize();
    ...
    private void testOwnUpdates() throws SQLException {
        ...
        for (int i = 0; i < 3; i++) {
            final int type = i == 0 ? ResultSet.TYPE_FORWARD_ONLY : i == 1 ? ResultSet.TYPE_SCROLL_INSENSITIVE : ResultSet.TYPE_SCROLL_SENSITIVE;
        ...
        final Statement stat = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    private void checkPrecision(final int expected, final String sql) throws SQLException {

        final ResultSetMetaData meta = stat.executeQuery(sql).getMetaData();
    ...
    private void testLimitMaxRows() throws SQLException {
        ...
        final ResultSet rs2 = stat.executeQuery("SELECT UPPER (C)  FROM one;");
    ...
    private void testInt() throws SQLException {
        ...
        assertResultSetMeta(rs, 3, new String[]{"ID", "VALUE", "N"}, new int[]{Types.INTEGER, Types.INTEGER, Types.NULL}, new int[]{10, 10, 1}, new int[]{0, 0, 0});
        rs.next();
    ...
    private void testVarchar() throws SQLException {
        ...
        assertResultSetMeta(rs, 2, new String[]{"ID", "VALUE"}, new int[]{Types.INTEGER, Types.VARCHAR}, new int[]{10, 255}, new int[]{0, 0});
        String value;
        rs.next();
    ...
    private void testDecimal() throws SQLException {
        ...
        rs = stat.executeQuery("SELECT * FROM TEST ORDER BY ID");
        assertResultSetMeta(rs, 2, new String[]{"ID", "VALUE"}, new int[]{Types.INTEGER, Types.DECIMAL}, new int[]{10, 10}, new int[]{0, 2});
        BigDecimal bd;
        rs.next();
    ...
    private void testDoubleFloat() throws SQLException {
        ...
        rs = stat.executeQuery("SELECT * FROM TEST ORDER BY ID");
        assertResultSetMeta(rs, 3, new String[]{"ID", "D", "R"}, new int[]{Types.INTEGER, Types.DOUBLE, Types.REAL}, new int[]{10, 17, 7}, new int[]{0, 0, 0});
        BigDecimal bd;
        rs.next();
        assertTrue(rs.getInt(1) == 1);
        assertTrue(!rs.wasNull());
    ...
    private void testDatetime() throws SQLException {
        ...
        assertResultSetMeta(rs, 2, new String[]{"ID", "VALUE"}, new int[]{Types.INTEGER, Types.TIMESTAMP}, new int[]{10, 23}, new int[]{0, 10});
        rs = stat.executeQuery("SELECT * FROM TEST ORDER BY ID");
        assertResultSetMeta(rs, 2, new String[]{"ID", "VALUE"}, new int[]{Types.INTEGER, Types.TIMESTAMP}, new int[]{10, 23}, new int[]{0, 10});
        rs.next();
        java.sql.Date date;
    ...
    private void testDatetimeWithCalendar() throws SQLException {
        ...
        final Calendar regular = Calendar.getInstance();
        Calendar other = null;
    ...
    private void testBlob() throws SQLException {
        ...
        stat.execute("INSERT INTO TEST VALUES(6,NULL)");
        rs = stat.executeQuery("SELECT * FROM TEST ORDER BY ID");
        assertResultSetMeta(rs, 2, new String[]{"ID", "VALUE"}, new int[]{Types.INTEGER, Types.BLOB}, new int[]{10, Integer.MAX_VALUE}, new int[]{0, 0});
        rs.next();
        ...
        assertTrue(!rs.wasNull());
        rs.next();
        final InputStream in = rs.getBinaryStream("value");
        final byte[] b = readAllBytes(in);
    ...
    private void testClob() throws SQLException {
        ...
        stat.execute("INSERT INTO TEST VALUES(7,NULL)");
        rs = stat.executeQuery("SELECT * FROM TEST ORDER BY ID");
        assertResultSetMeta(rs, 2, new String[]{"ID", "VALUE"}, new int[]{Types.INTEGER, Types.CLOB}, new int[]{10, Integer.MAX_VALUE}, new int[]{0, 0});
        rs.next();
    ...
    private byte[] readAllBytes(final InputStream in) throws SQLException {
    ...
    private void checkBytes(final byte[] test, final byte[] good) throws SQLException {
    ...
    private void checkColumnBigDecimal(final ResultSet rs, final int column, final int i, final String bd) throws SQLException {

        final BigDecimal bd1 = rs.getBigDecimal(column);
        final int i1 = rs.getInt(column);
        if (bd == null) {
