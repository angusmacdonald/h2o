...
public class TestStatement extends TestBase {
    ...
    private void testTraceError() throws SQLException {
        ...
        if (config.memory || config.networked || config.traceLevelFile != 0) { return; }
        final Statement stat = conn.createStatement();
        final File trace = new File(baseDir + "/statement.trace.db");
    ...
    private void testSavepoint() throws SQLException {
        ...
        final Savepoint savepoint1 = conn.setSavepoint();
        final int id1 = savepoint1.getSavepointId();
        try {
        ...
        final Savepoint savepoint2a = conn.setSavepoint();
        final Savepoint savepoint2 = conn.setSavepoint();
        ...
        final int id2 = savepoint2.getSavepointId();
        assertTrue(id1 != id2);
        stat.execute("UPDATE TEST SET NAME='Hallo' WHERE NAME='Hello'");
        final Savepoint savepointTest = conn.setSavepoint("Joe's");
        stat.execute("DELETE FROM TEST");
        ...
        conn.commit();
        final ResultSet rs = stat.executeQuery("SELECT NAME FROM TEST");
        rs.next();
    ...
    private void testStatement() throws SQLException {
        ...
        final Statement stat2 = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY, ResultSet.HOLD_CURSORS_OVER_COMMIT);
