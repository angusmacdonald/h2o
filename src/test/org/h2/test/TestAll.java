...
/**
 * The main test application. JUnit is not used because loops are easier to write in regular java applications (most tests are ran multiple
 * times using different settings).
 */
public class TestAll {
	
	/*
	 * Random test: java15 cd bin del *.db start cmd /k "java -cp .;%H2DRIVERS% org.h2.test.TestAll join >testJoin.txt" start cmd /k
	 * "java -cp . org.h2.test.TestAll synth >testSynth.txt" start cmd /k "java -cp . org.h2.test.TestAll all >testAll.txt" start cmd /k
	 * "java -cp . org.h2.test.TestAll random >testRandom.txt" start cmd /k "java -cp . org.h2.test.TestAll btree >testBtree.txt" start cmd
	 * /k "java -cp . org.h2.test.TestAll halt >testHalt.txt" java -cp . org.h2.test.TestAll crash >testCrash.txt java org.h2.test.TestAll
	 * timer
	 */
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * mark 1.1 as stable documentation: rolling review; consistently use <code> or not. test performance with log=2 use 127.0.0.1 if
		 * other addresses don't work select for update in mvcc mode: only lock the selected records? JCR: for each node type, create a
		 * table; one 'dynamic' table with parameter; option to cache the results Favicon per page <link rel="icon" type="image/png"
		 * href="/path/image.png"> create a short 4 pages documentation postgresql generate_series? is in-memory scan index re-using ids?
		 * don't store default values (store a special value) btree: maybe split at the insertion point split files (1 GB max size) add a
		 * setting (that can be changed at runtime) to call fsync and delay on each commit multithreaded kernel remove old TODO online
		 * backup may not work for very large files (document problem with jdk 1.4; document to use jar -xf) test web site with firefox 3,
		 * internet explorer, opera, safari, google chrome test with 1.0 documentation: how can you improve performance (group of settings)
		 * document url parameter open_new osgi: create a sample application, test, document merge join test case auto_reconnect
		 * implemented: - auto_server includes auto_reconnect - works with server mode - works with auto_server mode - keep temporary linked
		 * tables, variables on client - statements - prepared statements - small result sets (up to fetch size) - throws an error when
		 * autocommit is false - an error is thrown when the connection is lost while looping over large result sets (larger than fetch
		 * size) not implemented / not tested - batch updates - ignored in embedded mode - keep temporary tables (including data) on client
		 * - keep identity, getGeneratedKeys on client - throw error when in cluster mode TestMVCC: Concurrent update in table test: another
		 * transaction has updated or deleted the same row when exactly does it occur in other databases (PostgreSQL, Oracle)? Run benchmark
		 * with the newest versions of other databases. documentation: use 'server mode' not 'remote mode'. CREATE FUNCTION? Function
		 * interface find quote: You can't make a system that will not lose data, you can only make a system that knows the last save point
		 * of 100% integrity. There are too many variables and too much randomness on a cold hard power failure. not tested:
		 * PreparedProcedure PREPARE <name>(column,...) AS ... Procedure DeallocateProcedure DEALLOCATE [PLAN] <name> ExecuteProcedure
		 * EXECUTE <name>[([p[,...])] in help.csv, use complete examples for functions; add a test case analyzer configuration option for
		 * the fulltext search optimize where x not in (select): SELECT c FROM color LEFT OUTER JOIN (SELECT c FROM TABLE(c VARCHAR= ?)) p
		 * ON color.c = p.c WHERE p.c IS NULL; http://www.w3schools.com/sql/
		 */
		if ( args.length > 0 ) {
	...
	/**
	 * This method is called before a complete set of tests is run. It deletes old database files in the test directory and trace files. It
	 * also starts a TCP server if the test uses remote connections.
	 */
	public void beforeTest() throws SQLException {
		...
		if ( networked ) {
			String[] args = ssl ? new String[] { "-tcpSSL", "true", "-tcpPort", "9192" } : new String[] { "-tcpPort", "9192" };
	...
	private void printSystem() {
		...
		System.out.println("Java " + prop.getProperty("java.runtime.version") + ", " + prop.getProperty("java.vm.name") + ", "
				+ prop.getProperty("java.vendor"));
		System.out.println(prop.getProperty("os.name") + ", " + prop.getProperty("os.arch") + ", " + prop.getProperty("os.version") + ", "
				+ prop.getProperty("sun.os.patch.level") + ", " + prop.getProperty("file.separator") + " "
				+ prop.getProperty("path.separator") + " " + StringUtils.javaEncode(prop.getProperty("line.separator")) + " "
				+ prop.getProperty("user.country") + " " + prop.getProperty("user.language") + " " + prop.getProperty("user.variant") + " "
