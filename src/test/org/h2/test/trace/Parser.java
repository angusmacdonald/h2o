...
class Parser {
	...
	private void read() {
		...
		default:
			if ( Character.isLetter(ch) || ch == '_' ) {
				...
				while ( true ) {
					ch = line.charAt(pos);
					if ( Character.isDigit(ch) || ".+-eElLxabcdefABCDEF".indexOf(ch) >= 0 ) {
						pos++;
					} else {
						break;
					}
	...
	private void read(String s) {
		if ( !readIf(s) ) {
			throw new Error("Expected: " + s + " got: " + token + " in " + line);
		}
	}
	...
	private Arg parseValue() {
		if ( tokenType == STRING ) {
			...
			if ( readIf("true") ) {
				...
				throw new Error("Null: class not specified. Example: (java.lang.String)null");
			} else if ( readIf("new") ) {
				if ( readIf("String") ) {
					read("[");
					read("]");
			...
			throw new Error("Value expected, got: " + readToken() + " in " + line);
		}
	}
