...
/**
 * This class is a complete pure Java build tool. It allows to build this project without any external dependencies except a JDK.
 * Advantages: ability to debug the build, extensible, flexible, no XML, a bit faster.
 */
public class BuildBase {
	...
	public static class FileList extends ArrayList implements List {
		 ...
		 * 
		 * @param pattern
		 *            the pattern of the file names to remove
		 ...
		 * 
		 * @param pattern
		 *            the pattern of the file names to keep
		 * @return the new file list
		 */
	...
	/**
	 * This method is called if no other target is specified in the command line. The default behavior is to call projectHelp(). Override
	 * this method if you want another default behavior.
	 */
	protected void all() {
		projectHelp();
	}
	...
	protected void javadoc(String[] args) {
		...
		try {
			println("Javadoc");
			if ( quiet ) {
				System.setOut(filter(System.out, new String[] { "Loading source files for package", "Constructing Javadoc information",
						"Generating ", "Standard Doclet", "Building " }));
			}
	...
	/**
	 * Download a file if it does not yet exist. If no checksum is used (that is, if the parameter is null), the checksum is printed. For
	 * security, checksums should always be used.
	 * 
	 * @param target
	...
	protected void javac(String[] args, FileList files) {
		...
		try {
			if ( quiet ) {
				System.setErr(filter(System.err, new String[] { "Note:" }));
			}
	 ...
	 *            the class name
	 * @param args
	 *            the command line parameters to pass
	 */
