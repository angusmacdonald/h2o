...
/**
 * This tool checks that for each .java file there is a package.html file, that for each .java file there is at least one (class level)
 * javadoc comment, and that lines with comments are not too long.
 */
		...
		while ( true ) {
			...
			if ( inComment ) {
				if ( line.length() > MAX_COMMENT_LINE_SIZE && !line.trim().startsWith("* http://") ) {
					System.out.println("Long line : " + file.getAbsolutePath() + " (" + file.getName() + ":" + lineNumber + ")");
					errorCount++;
				}
				if ( line.endsWith("*/") ) {
					inComment = false;
