...
/**
 * This class updates the translation source code files by parsing the HTML documentation. It also generates the translated HTML
 * documentation.
 */
public class PrepareTranslation {
	...
	private static String extract(String documentName, File f, String target) throws Exception {
		...
		while ( true ) {
			...
			if ( event == XMLParser.END_DOCUMENT ) {
				...
				} else if ( "p".equals(tag) || "li".equals(tag) || "a".equals(tag) || "td".equals(tag) || "th".equals(tag)
						|| "h1".equals(tag) || "h2".equals(tag) || "h3".equals(tag) || "h4".equals(tag) || "body".equals(tag)
						|| "b".equals(tag) || "code".equals(tag) || "form".equals(tag) || "span".equals(tag) || "em".equals(tag) ) {
					if ( buff.length() == 0 ) {
				...
				} else if ( "p".equals(tag) || "li".equals(tag) || "td".equals(tag) || "th".equals(tag) || "h1".equals(tag)
						|| "h2".equals(tag) || "h3".equals(tag) || "h4".equals(tag) || "body".equals(tag) || "form".equals(tag) ) {
					if ( buff.length() > 0 ) {
						if ( templateIsCopy ) {
	...
	private void translateChunk(StringBuilder buff, int separator, String source, String target, HashMap keyMap, HashMap results) {
		buff.append(separator);
