...
public class Transaction {
	 ...
	 * @param tableName
	 */
	public Transaction(String transactionID, List<FutureTask<QueryResult>> executingQueries, List<CommitResult> recentlyCompletedQueries,
	...
	public boolean checkForCompletion(Database db) {
		...
		if ( incompleteQueries == null && transactionHasCommitted )
			return true;
		...
		for ( CommitResult completedQuery : newlyCompletedUpdates ) {
			...
			if ( completedQuery.isCommitQuery() ) {
				...
				if ( tableManager != null ) {
					try {
						tableManager.releaseLockAndUpdateReplicaState(true, db.getLocalDatabaseInstanceInWrapper(), newlyCompletedUpdates,
								true);
						...
						try {
							...
							tableManager.releaseLockAndUpdateReplicaState(true, db.getLocalDatabaseInstanceInWrapper(),
									newlyCompletedUpdates, true);
						} catch ( Exception e1 ) {
							e1.printStackTrace();
						}
					}
