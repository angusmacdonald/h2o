...
public class AsynchronousQueryExecutor {
	...
	public boolean executeQuery(String query, String transactionNameForQuery, Map<DatabaseInstanceWrapper, Integer> allReplicas,
		...
		for ( Entry<DatabaseInstanceWrapper, Integer> replicaToExecuteQueryOn : allReplicas.entrySet() ) {
			...
			executeQueryOnSpecifiedReplica(query, transactionNameForQuery, replicaToExecuteQueryOn.getKey(), replicaUpdateID,
					isReplicaLocal, parser, executingQueries, commitOperation, tableName);
			i++;
		}
	...
	private void executeQueryOnSpecifiedReplica(String sql, String transactionName, DatabaseInstanceWrapper replicaToExecuteQueryOn,
			Integer updateID, boolean isReplicaLocal, Parser parser, List<FutureTask<QueryResult>> executingQueries,
			boolean commitOperation, TableInfo tableInfo) {
	...
	private boolean waitUntilRemoteQueriesFinish(List<FutureTask<QueryResult>> incompleteQueries, int updatesNeededBeforeCommit,
		...
		database.getAsynchronousQueryManager().addTransaction(transactionNameForQuery, tableName, incompleteQueries,
				recentlyCompletedQueries, expectedUpdateID);
