 ...
 * system there will be one DatabaseInstanceRemote exposed via the instance's
 * RMI registry.
 * 
...
public interface DatabaseInstanceRemote extends H2ORemote, TwoPhaseCommit {
	...
	/**
	 * Get the connection information for this database instance, including the
	 * instances RMI port.
	 * 
	 ...
	 * 
	 * @return Object containing all connection information for the System
	 *         Table.
	 */
	 ...
	 * provided with this method call the database instance must request the
	 * locks needed to execute this query.
	 * 
	 * @param sql
	 *            The query to be executed.
	 * @param systemTableCommand
	 *            True if this command is to update a System Table replica;
	 *            otherwise false. This is done to prevent deadlock, where there
	 ...
	 *             Thrown if there was an error in the queries execution.
	 */
	int executeUpdate(String sql, boolean systemTableCommand)
			throws RemoteException, SQLException;

	/**
	 * Set the current location of the System Table. This is typically only
	 * called by the LookupPinger thread to continually inform the node
	 * responsible for #(SM) of the System Tables location.
	 * 
	 * @param systemTableLocation
	 *            The location in Chord of the System Table.
	 * @param databaseURL
	 *            Object containing all connection information for the System
	 *            Table.
	 * @throws RemoteException
	 *             Thrown if there were problems connecting to the instance.
	 */
	...
	/**
	 * Look for a reference to the specified Table Manager. This may be called
	 * by a System Table which has just been re-instantiated from persisted
	 * state, and which doesn't have direct pointers to Table Manager proxies
	 * (only their location).
	 * 
	 * @param tableInfo
	 *            The table name and schema name of the table to be found.
	 * @return Remote reference to the Table Manager, or null if nothing was
	 *         found.
	 * @throws RemoteException
	 *             Thrown if there were problems connecting to the instance.
	 */
	public TableManagerRemote findTableManagerReference(TableInfo tableInfo)
			throws RemoteException;
	 ...
	 * 
	 * @param databaseURL
	 *            The location on which the table manager was previously held.
	 *            Used to find the correct meta-table.
