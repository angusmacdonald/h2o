...
public class ChordRemote implements IDatabaseRemote, IChordInterface, Observer {
    ...
    private boolean attemptToJoinChordRing(LocalH2OProperties persistedInstanceInformation, DatabaseURL localMachineLocation, List<String> databaseInstances) {
        ...
        for (String url : databaseInstances) {
            ...
            boolean connected = joinChordRing(localMachineLocation.getHostname(), portToJoinOn, instanceURL.getHostname(), instanceURL.getRMIPort(), localMachineLocation.sanitizedLocation());
    ...
    public DatabaseInstanceRemote getDatabaseInstanceAt(DatabaseURL dbURL) throws RemoteException {
        ...
        if (dbURL.equals(this.localMachineLocation)) { return this.getLocalDatabaseInstance(); }
    ...
    private boolean startChordRing(String hostname, int port, DatabaseURL databaseURL) {
        ...
        Diagnostic.traceNoEvent(DiagnosticLevel.INIT, "Started local Chord node on : " + databaseURL.sanitizedLocation() + " : " + hostname + ":" + port + " : initialized with key :" + chordNode.getKey().toString(10) + " : " + chordNode.getKey() + " : System Table at " + this.systemTableRef.getLookupLocation() + " : ");
    ...
    private boolean joinChordRing(String localHostname, int localPort, String remoteHostname, int remotePort, String databaseName) {
        ...
        Diagnostic.traceNoEvent(DiagnosticLevel.INIT, "Started local Chord node on : " + databaseName + " : " + localHostname + " : " + rmiPort + " : initialized with key :" + chordNode.getKey().toString(10) + " : " + chordNode.getKey() + " : System Table at " + this.systemTableRef.getLookupLocation() + " : " + chordNode.getSuccessor().getKey());
    ...
    public void update(Observable o, Object arg) {
         ...
         */
        if (arg.equals(IChordNode.PREDECESSOR_CHANGE_EVENT)) predecessorChangeEvent();
        else if (arg.equals(IChordNode.SUCCESSOR_CHANGE_EVENT)) successorChangeEvent();
    }
