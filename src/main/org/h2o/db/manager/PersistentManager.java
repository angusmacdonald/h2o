...
public abstract class PersistentManager {
	 ...
	 * @throws Exception
	 */
	public PersistentManager(Database db, String tables, String replicas, String connections, String tableManagerRelation,
	...
	public static String createSQL(String tables, String connections) {
		...
		sql += "CREATE TABLE IF NOT EXISTS " + connections + "(" + "connection_id INTEGER NOT NULL auto_increment(1,1),"
				+ "connection_type VARCHAR(5), " + "machine_name VARCHAR(255)," + "db_location VARCHAR(255),"
				+ "connection_port INT NOT NULL, " + "chord_port INT NOT NULL, " + "active BOOLEAN, " + "PRIMARY KEY (connection_id) );";
		
		sql += "CREATE SCHEMA IF NOT EXISTS H2O; " + "\n\nCREATE TABLE IF NOT EXISTS " + tables
				+ "( table_id INT NOT NULL auto_increment(1,1), " + "schemaname VARCHAR(255), tablename VARCHAR(255), "
				+ "last_modification INT NOT NULL, " + "manager_location INTEGER NOT NULL, " + "PRIMARY KEY (table_id), "
	 ...
	 * @throws SQLException
	 */
	public boolean addTableInformation(DatabaseURL tableManagerURL, TableInfo tableDetails, boolean addReplicaInfo) throws RemoteException,
	...
	public void persistInactiveInformation(TableInfo tableDetails, Set<DatabaseInstanceWrapper> newlyInactiveReplicas) {
		if ( newlyInactiveReplicas == null || newlyInactiveReplicas.size() == 0 )
			return;
	...
	public boolean connectionInformationExists(DatabaseURL dbURL) throws SQLException {
		String sql = "SELECT count(connection_id) FROM " + connectionRelation + " WHERE machine_name='" + dbURL.getHostname()
				+ "' AND db_location='" + dbURL.getDbLocation() + "' AND connection_port=" + dbURL.getPort() + " AND connection_type='"
	...
	public int addConnectionInformation(DatabaseURL dbURL, boolean isActive) throws SQLException {
		...
		if ( connectionInformationExists(dbURL) ) {
			...
			sql = "\nUPDATE " + connectionRelation + " SET chord_port = " + dbURL.getRMIPort() + ", active ='" + isActive
			...
			sql = "\nINSERT INTO " + connectionRelation + " VALUES (null, '" + connection_type + "', '" + dbURL.getHostname() + "', '"
					+ dbURL.getDbLocation() + "', " + dbURL.getPort() + ", " + dbURL.getRMIPort() + ", " + isActive + ");\n";
	...
	public int getNumberofReplicas(String tableName, String schemaName) {
		String sql = "SELECT count(*) FROM " + replicaRelation + ", " + tableRelation + " WHERE tablename = '" + tableName
				+ "' AND schemaname='" + schemaName + "' AND" + " " + tableRelation + ".table_id=" + replicaRelation + ".table_id;";
	...
	public int getConnectionID(DatabaseURL dbURL) {
		...
		String sql = "SELECT connection_id FROM " + connectionRelation + " WHERE machine_name='" + machine_name + "' AND connection_port="
				+ connection_port + " AND connection_type='" + connection_type + "' AND db_location = '" + dbURL.getDbLocation() + "';";
	...
	public boolean isTableListed(TableInfo ti) throws SQLException {
		String sql = "SELECT count(*) FROM " + tableRelation + " WHERE tablename='" + ti.getTableName() + "' AND schemaname='"
				+ ti.getSchemaName() + "';";
	...
	public boolean isReplicaListed(TableInfo ti, int connectionID) throws SQLException {
		String sql = "SELECT count(*) FROM " + replicaRelation + ", " + tableRelation + ", " + connectionRelation + " WHERE tablename='"
				+ ti.getTableName() + "' AND schemaname='" + ti.getSchemaName() + "' AND " + tableRelation + ".table_id=" + replicaRelation
				+ ".table_id AND " + replicaRelation + ".connection_id = " + connectionRelation + ".connection_id AND "
	...
	private int addTableInformation(TableInfo tableInfo, int connectionID) throws SQLException {
		String sql = "INSERT INTO " + tableRelation + " VALUES (null, '" + tableInfo.getSchemaName() + "', '" + tableInfo.getTableName()
				+ "', " + tableInfo.getModificationID() + ", " + connectionID + ");";
		return executeUpdate(sql);
	}
	
	protected int addTableManagerReplicaInformation(int tableID, int connectionID, int primaryLocationConnectionID, boolean active)
			throws SQLException {
		
		return metaDataReplicaManager.addTableManagerReplicaInformation(tableID, connectionID, primaryLocationConnectionID, active);
	}
	
	protected void addTableManagerReplicaInformationOnCreateTable(int tableID, int connectionID, boolean active,
	...
	private int addReplicaInformation(TableInfo ti, int tableID, int connectionID) throws SQLException {
		...
		String sql = "INSERT INTO " + replicaRelation + " VALUES (null, " + tableID + ", " + connectionID + ", '" + ti.getTableType()
				+ "', true, " + ti.getTableSet() + ");\n";
		return executeUpdate(sql);
	}
	...
	protected int toggleReplicasActive(int tableID, int connectionID, boolean active) throws SQLException {
		...
		String sql = "UPDATE " + replicaRelation + " SET active=" + active + " WHERE table_id=" + tableID + " AND connection_id="
				+ connectionID + ";\n";
	...
	public void removeConnectionInformation(DatabaseInstanceRemote databaseInstance) throws RemoteException, MovedException {
		...
		String sql = "\nUPDATE " + connectionRelation + " SET active = false WHERE machine_name='" + dburl.getHostname()
	...
	public void changeTableManagerLocation(TableInfo tableInfo) {
		...
		String sql = "\nUPDATE " + tableRelation + " SET manager_location = " + connectionID + " WHERE schemaname='"
				+ tableInfo.getSchemaName() + "' AND tablename='" + tableInfo.getTableName() + "';";
