...
public class TableManager extends PersistentManager implements TableManagerRemote, AutonomicController, Migratable {
	...
	public static int createTableManagerTables(Session session) throws SQLException {
		...
		sql += "\n\nCREATE TABLE IF NOT EXISTS " + getMetaTableName(databaseName, TableManager.REPLICAS) + "("
				...
				+ "storage_type VARCHAR(255), " + "active boolean NOT NULL, " + "table_set INT NOT NULL, " + "PRIMARY KEY (replica_id), "
				+ "FOREIGN KEY (table_id) REFERENCES " + getMetaTableName(databaseName, TableManager.TABLES)
	...
	public synchronized QueryProxy getQueryProxy(LockType lockRequested, DatabaseInstanceWrapper databaseInstanceWrapper)
		...
		QueryProxy qp = new QueryProxy(lockGranted, tableInfo, selectReplicaLocations(lockRequested, databaseInstanceWrapper, isDrop),
				this, databaseInstanceWrapper, currentUpdateID, lockRequested);
	...
	private Map<DatabaseInstanceWrapper, Integer> selectReplicaLocations(LockType lockType, DatabaseInstanceWrapper requestingDatabase,
		...
		if ( lockType == LockType.CREATE ) {
			...
			if ( isDrop ) {
				...
				replicaLocations = this.replicaManager.getAllReplicas(); // The update could be sent to any or all machines holding the
																			// given table.
			}
	...
	public void releaseLockAndUpdateReplicaState(boolean commit, DatabaseInstanceWrapper requestingDatabase,
			Collection<CommitResult> committedQueries, boolean asynchronousCommit) throws RemoteException, MovedException {
	...
	private void printCurrentActiveReplicas() {
		if ( Diagnostic.getLevel().equals(DiagnosticLevel.INIT) ) {
			...
			String sql = "SELECT LOCAL ONLY " + "connection_type, machine_name, db_location, connection_port, chord_port, "
					...
					+ ".connection_id FROM " + getMetaTableName(databaseName, REPLICAS) + ", " + getMetaTableName(databaseName, TABLES)
					+ ", " + getMetaTableName(databaseName, CONNECTIONS) + " WHERE tablename = '" + tableName + "' AND schemaname='"
					+ schemaName + "' AND " + getMetaTableName(databaseName, REPLICAS) + ".active='true' AND "
					+ getMetaTableName(databaseName, TABLES) + ".table_id=" + getMetaTableName(databaseName, REPLICAS) + ".table_id AND "
					+ getMetaTableName(databaseName, REPLICAS) + ".connection_id=" + getMetaTableName(databaseName, CONNECTIONS)
					+ ".connection_id;";
			...
			try {
				...
				while ( rs.next() ) {
					...
					Diagnostic.traceNoEvent(DiagnosticLevel.INIT, "\tLocation: " + dbURL + "; tableID = " + rs.currentRow()[5].getString()
							+ "; connectionID = " + rs.currentRow()[6].getString());
	...
	public void recreateReplicaManagerState(String oldPrimaryDatabaseName) throws RemoteException, SQLException {
		...
		String sql = "SELECT LOCAL ONLY connection_type, machine_name, db_location, connection_port, chord_port FROM " + oldReplicaRelation
				...
				+ schemaName + "' AND " + oldReplicaRelation + ".active='true' AND " + oldTableRelation + ".table_id=" + oldReplicaRelation
				+ ".table_id AND " + oldconnectionRelation + ".connection_id=" + oldReplicaRelation + ".connection_id;";
