...
public class TableManager extends PersistentManager implements TableManagerRemote, AutonomicController, Migratable {
    ...
    public synchronized QueryProxy getQueryProxy(LockType lockRequested, DatabaseInstanceWrapper databaseInstanceWrapper) throws RemoteException, SQLException, MovedException {
        ...
        QueryProxy qp = new QueryProxy(lockGranted, tableInfo, selectReplicaLocations(lockRequested, databaseInstanceWrapper, isDrop), this, databaseInstanceWrapper, currentUpdateID, lockRequested);
    ...
    private Map<DatabaseInstanceWrapper, Integer> selectReplicaLocations(LockType lockType, DatabaseInstanceWrapper requestingDatabase, boolean isDrop) {
        ...
        if (lockType == LockType.READ || lockType == LockType.NONE) { return this.replicaManager.getActiveReplicas(); }// else, a more informed decision is needed.
        ...
        if (lockType == LockType.CREATE) {
            ...
            if (potentialReplicaLocations != null && potentialReplicaLocations.size() > 0) {
                ...
                for (DatabaseInstanceWrapper dbInstance : potentialReplicaLocations) {
                    ...
                    if (nullIfNoPrevious == null) currentReplicationFactor++;
     ...
     */
    @Override
    public void releaseLockAndUpdateReplicaState(boolean commit, DatabaseInstanceWrapper requestingDatabase, Collection<CommitResult> committedQueries, boolean asynchronousCommit) throws RemoteException, MovedException {
        ...
        if (Diagnostic.getLevel().equals(DiagnosticLevel.INIT)) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    DatabaseURL dbURL = new DatabaseURL(rs.currentRow()[0].getString(), rs.currentRow()[1].getString(), rs.currentRow()[3].getInt(), rs.currentRow()[2].getString(), false, rs.currentRow()[4].getInt());
