...
/**
 * Used to find an existing database system by connecting through the set of
 * known nodes (system table nodes).
 * 
...
public class H2OLocatorInterface {
	...
	/**
	 * Connections to locator servers and the corresponding last known update
	 * count on that server.
	 */
	 ...
	 */
	public H2OLocatorInterface(String databaseName, String descriptorURL)
	...
	/**
	 * Get the set of database locations which hold valid, up-to-date, system
	 * table state.
	 * 
	 * @return Database URLs represented as strings, where system table state is
	 *         stored.
	 * @throws IOException
	 *             Thrown if the method was unable to connect to a locator
	 *             server.
	 */
	public List<String> getLocations() throws IOException {
		 ...
		 */
		for (Entry<LocatorClientConnection, Integer> locatorLocation : locatorConnections
				.entrySet()) {
		...
		if (!foundMajority) {
			...
			int currentMax = 0;
			for (Entry<ReplicaLocationsResponse, Integer> entry : matches
					.entrySet()) {
			...
			if (currentMax < (resultList.size() / 2) + 1)
	...
	/**
	 * Update the set of valid system table replica locations on the locator
	 * files.
	 * 
	 * @param replicaLocations
	 *            Database URLs represented as strings, where system table state
	 *            is stored.
	 * @throws IOException
	 *             Thrown if the method was unable to connect to a locator
	 *             server.
	...
	public boolean lockLocators(String databaseInstanceString)
			throws IOException, StartupException {
		int successful = 0;

		if (locatorLocations.length < MINIMUM_NUMER_OF_LOCATOR_SERVERS) {
			throw new StartupException(
					"Not enough locator servers to reach majority consensus.");
		}
		...
		for (Entry<LocatorClientConnection, Integer> locatorLocation : locatorConnections
			...
			if (locatorLocation.getValue() == 0) {
				throw new StartupException(
						"Update count for a locator server is still set at 0. A get message should have"
	...
	public boolean commitLocators(String databaseInstanceString)
			throws IOException, StartupException {
		int successful = 0;
		...
		for (Entry<LocatorClientConnection, Integer> locatorLocation : locatorConnections
				.entrySet()) {
			LocatorClientConnection lcc = locatorLocation.getKey();
			boolean unlocked = lcc
					.confirmSystemTableCreation(databaseInstanceString);
			if (unlocked)
				successful++;
		}
	 ...
	 */
	private LocatorClientConnection getLocatorConnection(String locatorLocation)
			throws IOException {
		...
		try {
			...
			throw new IOException(
					"Failed to parse locator location from database descriptor. Ensure the descriptor file lists locators as host:port combinations.");
		}
	...
	/**
	 * Whether a majority has been achieved. Whether successfulResponses is
	 * greater than or equal to half of numberOfLocators + 1.
	 * 
	 * This is public to allow junit tests to access it.
	 */
	public static boolean hasAchievedMajority(int successfulResponses,
