...
public class LocatorClientConnection {
	...
	private void setupSocketConnection() throws IOException {
		try {
			...
			throw new IOException("Locator server was not found. Make sure server is running on " + hostname + ":" + port
					+ ". Program will now terminate (original error: " + e.getMessage() + ").");
		}
	}
	
	/**
	 * If the socket is not currently conencted this method will try to make another connection. If this fails it will inform the calling
