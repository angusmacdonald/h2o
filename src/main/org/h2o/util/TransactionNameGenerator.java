...
public class TransactionNameGenerator {
	...
	/**
	 * Used when no database name is specified to avoid unnecessarily contacting
	 * local instance of this class.
	 */
	...
	private String generatePrefix(DatabaseURL url) {
		...
		if (url == null) {
			...
			part = (url.isTcp() ? hostname.replace(".", "") + url.getPort()
	...
	/**
	 * Generate a unique name for a new transaction based on the identity of the
	 * requesting database.
	 * 
	...
	/**
	 * Generate a unique name for a new local transaction. Transactions spanning
	 * multiple databases should used the other method.
	 * 
	 * @param requestingDatabase
	 * @param tableName
