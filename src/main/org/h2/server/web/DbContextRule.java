...
public class DbContextRule implements Rule {
    ...
    private void addColumn(Sentence sentence) {
        ...
        if (tableName == null) {
            if (set == null && aliases == null) { return; }
            if ((set != null && set.size() > 1) || (aliases != null && aliases.size() > 1)) { return; }
        }
        if (table == null) {
    ...
    public boolean matchRemove(Sentence sentence) {
        ...
        if (sentence.getQuery().length() == 0) { return false; }
        String s;
        switch (type) {
            case SCHEMA:
    ...
    private String matchColumnAlias(Sentence sentence) {
        ...
        if (Parser.isKeyword(alias, true)) { return null; }
    ...
    private String matchTableAlias(Sentence sentence, boolean add) {
        ...
        if ((map != null && map.containsKey(alias)) || (sentence.getLastTable() == null)) {
            if (add && query.length() == alias.length()) { return query; }
