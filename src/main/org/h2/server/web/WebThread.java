...
class WebThread extends Thread implements DatabaseEventListener {
	...
	private int addColumns(DbTableOrView table, StringBuilder buff, int treeIndex, boolean showColumnTypes, StringBuilder columnsBuffer,
			int indentationLevel, int tableLevel) {
		...
		for ( int i = 0; columns != null && i < columns.length; i++ ) {
			...
			// indentationLevel = 2;
			buff.append("setNode(" + treeIndex + ", " + ( indentationLevel + 1 ) + ", " + tableLevel + 1 + ", 'column', '"
					+ PageParser.escapeJavaScript(column.name) + "', 'javascript:ins(\\'" + col + "\\')');\n");
			treeIndex++;
			if ( showColumnTypes ) {
				buff.append("setNode(" + treeIndex + ", " + ( indentationLevel + 2 ) + ", " + ( tableLevel + 2 ) + ", 'type', '"
						+ PageParser.escapeJavaScript(column.dataType) + "', null);\n");
				treeIndex++;
			}
		}
		return treeIndex;
	...
	private int addTablesAndViews(DbSchema schema, boolean mainSchema, StringBuilder buff, int treeIndex) throws SQLException {
		...
		for ( Entry<String, Set<DbTableOrView>> entrySet : tables.entrySet() ) {
			...
			if ( tableSet.size() > 1 ) {
				...
				buff.append("setNode(" + treeIndex + ", " + indentLevel + ", " + tableLevel + ", 'index', ' "
						+ PageParser.escapeJavaScript(tableName + " [" + tableSet.size() + " copies]") + "', 'javascript:ins(\\'"
						+ schema.name + "." + tableName + "\\',true)');\n");
				treeIndex++;
				// tableLevel++;
				indentLevel++;
			}
			...
			for ( DbTableOrView table : tableSet ) {
				...
				buff.append("setNode(" + treeIndex + ", " + indentLevel + ", " + tableLevel + ", 'table', '" + isLink + " "
						+ PageParser.escapeJavaScript(table.name + ( ( tableSet.size() == 1 ) ? " [1 copy]" : "" ))
					...
					if ( schema.contents.isH2 ) {
						...
						try {
							...
							if ( rs.next() ) {
								...
								buff.append("setNode(" + treeIndex + indentNode + " 'type', '" + PageParser.escapeJavaScript(sql)
										+ "', null);\n");
								treeIndex++;
							}
							rs.close();
	...
	private String login() {
		...
		if ( socket != null && url.startsWith("jdbc:h2:") && !url.startsWith("jdbc:h2:tcp:") && !url.startsWith("jdbc:h2:ssl:")
