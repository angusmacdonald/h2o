...
public class RuleElement implements Rule {
    ...
    public String random(Bnf config, int level) {
        ...
        if (keyword) { return name.length() > 1 ? " " + name + " " : name; }
        if (link != null) { return link.random(config, level + 1); }
    ...
    public void setLinks(HashMap ruleMap) {
        ...
        if (link == null) { throw new Error(">>>" + name + "<<<"); }
    }
    ...
    public boolean matchRemove(Sentence sentence) {
        ...
        if (!link.matchRemove(sentence)) { return false; }
