...
public class Bnf {
    ...
    private RuleHead addRule(String topic, String section, Rule rule) {
        ...
        if (ruleMap.get(StringUtils.toLowerEnglish(topic)) != null) { throw new Error("already exists: " + topic); }
    ...
    private Rule parseToken() {
        ...
        if ((firstChar >= 'A' && firstChar <= 'Z') || (firstChar >= 'a' && firstChar <= 'z')) {
            ...
            if (firstChar != ']') { throw new Error("expected ], got " + currentToken + " syntax:" + syntax); }
        }
        else if (firstChar == '{') {
            read();
            r = parseOr();
