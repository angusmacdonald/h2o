...
public class UpdatableRow {
    ...
    public UpdatableRow(JdbcConnection conn, ResultInterface result) throws SQLException {
        ...
        for (int i = 0; i < columnCount; i++) {
            ...
            else if (!tableName.equals(t)) { return; }
            if (schemaName == null) {
                schemaName = s;
            }
            else if (!schemaName.equals(s)) { return; }
        }
        ResultSet rs = meta.getTables(null, JdbcUtils.escapeMetaDataPattern(schemaName), JdbcUtils.escapeMetaDataPattern(tableName), new String[]{"TABLE"});
        if (!rs.next()) { return; }
    ...
    private int getColumnIndex(String columnName) throws SQLException {
        ...
        for (int i = 0; i < columnCount; i++) {
            ...
            if (col.equals(columnName)) { return i; }
        }
    ...
    public Value[] readRow(Value[] row) throws SQLException {
        ...
        setKey(prep, 1, row);
        ResultSet rs = prep.executeQuery();
        if (!rs.next()) { throw Message.getSQLException(ErrorCode.NO_DATA_AVAILABLE); }
    ...
    public void insertRow(Value[] row) throws SQLException {
        ...
        int count = prep.executeUpdate();
        if (count != 1) { throw Message.getSQLException(ErrorCode.NO_DATA_AVAILABLE); }
    }
