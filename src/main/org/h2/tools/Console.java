...
/**
 * This tool starts the H2 Console (web-) server, as well as the TCP and PG server. For JDK 1.6, a system tray icon is created, for
 * platforms that support it. Otherwise, a small window opens.
 * 
...
// ## AWT begin ##
	...
	/**
	 * The command line interface for this tool. The command line options are the same as in the Server tool, but this tool will always
	 * start the TCP, TCP and PG server. Options are case sensitive.
	 * 
	...
	private boolean createTrayIcon() {
		try {
			...
			Boolean supported = (Boolean) Class.forName("java.awt.SystemTray").getMethod("isSupported", new Class[0])
			...
			Dimension d = (Dimension) Class.forName("java.awt.SystemTray").getMethod("getTrayIconSize", new Class[0])
			...
			// menuConsole);
			Object trayIcon = Class.forName("java.awt.TrayIcon").getConstructor(new Class[] { Image.class, String.class, PopupMenu.class })
