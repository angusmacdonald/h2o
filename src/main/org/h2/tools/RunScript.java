...
/**
 * Executes the contents of a SQL script file against a database. This tool is usually used to create a database from script. It can also be
 * used to analyze performance problems by running the tool using Java profiler settings such as:
 * 
 * <pre>
...
public class RunScript extends Tool {
	 ...
	 * <li>-options (to specify a list of options ;only for H2 and only when using the embedded mode)</li>
	 * </ul>
	...
	private void process(Connection conn, boolean continueOnError, String path, Reader reader, String charsetName) throws SQLException,
			IOException {
		Statement stat = conn.createStatement();
	 ...
	 */
	public static void execute(String url, String user, String password, String fileName, String charsetName, boolean continueOnError)
			throws SQLException {
	 ...
	 *            if execution should be continued if an error occurs
	 */
	void process(String url, String user, String password, String fileName, String charsetName, boolean continueOnError)
			throws SQLException {
		try {
			org.h2.Driver.load();
	...
	/**
	 * If results of statements should be cross-checked with the expected output. The expected result is the next line(s) of the script,
	 * commented.
	 * 
	 * @param check
	 *            true if yes
