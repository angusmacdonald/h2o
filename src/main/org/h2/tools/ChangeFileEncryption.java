...
/**
 * A tools to change, remove or set a file password of a database without opening it. The encryption algorithm can be changed as well. It
 * can not be used to change a password of a user.
 */
public class ChangeFileEncryption extends Tool {
    ...
    /**
     * Get the file encryption key for a given password. The password must be supplied as char arrays and is cleaned in this method.
     * 
     * @param password
    ...
    private void process(String dir, String db, String cipher, char[] decryptPassword, char[] encryptPassword, boolean quiet) throws SQLException {
        ...
        if (encryptPassword != null) {
            for (char element : encryptPassword) {
                if (element == ' ') { throw new SQLException("The file password may not contain spaces"); }
            }
        }
        change.out = out;
        change.dir = dir;
