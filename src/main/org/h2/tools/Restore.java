...
public class Restore extends Tool {
    ...
    private static String getOriginalDbName(String fileName, String db) throws IOException {
        ...
        try {
            ...
            zipIn.close();
            if (multiple && !originalDbName.equals(db)) { throw new IOException("Multiple databases found, but not " + db); }
    ...
    private void process(String zipFileName, String directory, String db) throws SQLException {
        ...
        InputStream in = null;
        try {
            if (!FileUtils.exists(zipFileName)) { throw new IOException("File not found: " + zipFileName); }
            String originalDbName = null;
            if (db != null) {
                originalDbName = getOriginalDbName(zipFileName, db);
                if (originalDbName == null) { throw new IOException("No database named " + db + " found"); }
