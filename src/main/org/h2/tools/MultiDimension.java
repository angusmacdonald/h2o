...
/**
 * A tool to help an application execute multi-dimensional range queries. The algorithm used is database independent, the only requirement
...
public class MultiDimension {
    ...
    /**
     * Convert the multi-dimensional value into a one-dimensional (scalar) value. This is done by interleaving the bits of the values. Each
     * values must be bigger or equal to 0. The maximum value is dependent on the number of dimensions. For two keys, it is 32 bit, for 3:
     * 21 bit, 4: 16 bit, 5: 12 bit, 6: 10 bit, 7: 9 bit, 8: 8 bit.
     * 
     * @param values
    ...
    public long interleave(int[] values) {
        ...
        for (int i = 0; i < dimensions; i++) {
            long k = values[i];
            if (k < 0 || k > max) { throw new IllegalArgumentException("value out of range; value=" + values[i] + " min=0 max=" + max); }
        ...
        if (dimensions == 2) {
            ...
            if (xx != x) { throw new IllegalArgumentException("test"); }
        }
        return x;
    }
    ...
    /**
     * Generates an optimized multi-dimensional range query. This query is database independent, however the performance is not as good as
     * when using generatePreparedQuery
     * 
     * @param table
     *            the table name
     * @param columns
    ...
    /**
     * Gets a list of ranges to be searched for a multi-dimensional range query where min &lt;= value &lt;= max. In most cases, the ranges
     * will be larger than required in order to combine smaller ranges into one. Usually, about double as much points will be included in
     * the resulting range.
     * 
     * @param min
     *            the minimum value
    ...
    private long[][] getMortonRanges(int[] min, int[] max) {
        ...
        int len = min.length;
        if (max.length != len) { throw new IllegalArgumentException("dimensions mismatch"); }
        for (int i = 0; i < len; i++) {
            if (min[i] > max[i]) {
                int temp = min[i];
                min[i] = max[i];
    ...
    private void addMortonRanges(ArrayList list, int[] min, int[] max, int len, int level) {
        ...
        if (level > 100) { throw new IllegalArgumentException("Stop"); }
        int largest = 0, largestDiff = 0;
        long size = 1;
        for (int i = 0; i < len; i++) {
            int diff = max[i] - min[i];
            if (diff < 0) { throw new IllegalArgumentException("Stop"); }
            size *= diff + 1;
            if (size < 0) { throw new IllegalArgumentException("Stop"); }
            if (diff > largestDiff) {
                largestDiff = diff;
                largest = i;
            }
        ...
        if (high < low) { throw new IllegalArgumentException("Stop"); }
    ...
    private int findMiddle(int a, int b) {
        ...
        if (m <= a || m >= b) { throw new IllegalArgumentException("stop"); }
        return m;
    }
