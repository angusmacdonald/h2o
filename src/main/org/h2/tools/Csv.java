...
public class Csv implements SimpleRowSource {
    ...
    /**
     * Reads from the CSV file and returns a result set. The rows in the result set are created on demand, that means the file is kept open
    ...
    /**
     * Reads CSV data from a reader and returns a result set. The rows in the result set are created on demand, that means the reader is
     * kept open until all rows are read or the result set is closed.
     * 
     * @param reader
     *            the reader
     * @param colNames
    ...
    private String escape(String data) {
        ...
        if (data.indexOf(fieldDelimiter) < 0) {
            if (escapeCharacter == fieldDelimiter || data.indexOf(escapeCharacter) < 0) { return data; }
        }
    ...
    /**
     * Set the escape character (used to escape the field delimiter). The default is " (a double quote). The value 0 means no escape
     * character is used.
     * 
