...
public class Recover extends Tool implements DataHandler {
    ...
    private String getSQL(Value v) {
        ...
        if (v instanceof ValueLob) {
            ...
            if (small == null) {
                ...
                if (lob.getType() == Value.BLOB) { return "READ_BLOB('" + file + ".txt')"; }
    ...
    private void dumpPageLogStream(PrintWriter writer, FileStore store, int logHead, int pageSize) throws IOException, SQLException {
        ...
        while (true) {
            ...
            if (x == PageLog.NO_OP) {
                ...
                writer.println("-- session " + sessionId + " table " + storageId + " " + (x == PageLog.ADD ? "add" : "remove") + " " + row.toString());
            }
    ...
    static class PageInputStream extends InputStream {
        ...
        private void fillBuffer() throws IOException {
            ...
            if (remaining > 0 || endOfFile) { return; }
            if (nextPage == 0) {
                endOfFile = true;
                return;
            }
            ...
            try {
                ...
                if (type != t || p != parentPage) {
                    writer.println("-- ERROR  page:" + nextPage + " type:" + t + " parent:" + p + " expected type:" + type + " expected parent:" + parentPage);
                }
                parentPage = nextPage;
                if (last) {
                    nextPage = 0;
