...
/**
 * Dumps the contents of a database file to a human readable text file. This text file can be used to recover most of the data. This tool
 * does not open the database and can be used even if the database files are corrupted. A database can get corrupted if there is a bug in
 * the database engine or file system software, or if an application writes into the database file that doesn't understand the the file
 * format, or if there is a hardware problem.
 */
public class Recover extends Tool implements DataHandler {
	...
	private void writeDataError(PrintWriter writer, String error, byte[] data, int dumpBlocks) {
		writer.println("-- ERROR: " + error + " block:" + block + " blockCount:" + blockCount + " storageId:" + storageId
		...
		try {
			...
			while ( true ) {
				...
				if ( type == 'P' ) {
					...
					case 'T':
						writer.println("//   truncate session:" + sessionId + " storage:" + storageId + " pos:" + recId + " blockCount:"
								+ blockCount);
						break;
					case 'I':
						writer.println("//   insert session:" + sessionId + " storage:" + storageId + " pos:" + recId + " blockCount:"
								+ blockCount);
						if ( storageId >= 0 ) {
						...
						break;
					case 'D':
						writer.println("//   delete session:" + sessionId + " storage:" + storageId + " pos:" + recId + " blockCount:"
								+ blockCount);
						if ( storageId >= 0 ) {
							writeLogRecord(writer, s);
						}
						break;
					default:
						writer.println("//   type?:" + type + " session:" + sessionId + " storage:" + storageId + " pos:" + recId
		...
		try {
			...
			for ( block = 0; block < blocks; block += blockCount ) {
				...
				if ( pageOwners[page] != 0 && pageOwners[page] != storageId ) {
					writeDataError(writer, "double allocation, previous=" + pageOwners[page] + " now=" + storageId, s.getBytes(),
							blockCount);
				} else {
	...
	private void dumpPageLogStream(PrintWriter writer, FileStore store, int logHead, int pageSize) throws IOException, SQLException {
		...
		DataInputStream in = new DataInputStream(new PageInputStream(writer, this, store, logHead, pageSize, 0, Page.TYPE_LOG));
		...
		while ( true ) {
			...
			if ( x == PageLog.NO_OP ) {
				...
				writer.println("-- session " + sessionId + " table " + storageId + " " + ( x == PageLog.ADD ? "add" : "remove" ) + " "
						+ row.toString());
	...
	static class PageInputStream extends InputStream {
		...
		public PageInputStream(PrintWriter writer, DataHandler handler, FileStore store, int firstPage, int pageSize, int parent, int type) {
			this.writer = writer;
			this.store = store;
		...
		private void fillBuffer() throws IOException {
			...
			try {
				...
				if ( type != t || p != parentPage ) {
					writer.println("-- ERROR  page:" + nextPage + " type:" + t + " parent:" + p + " expected type:" + type
	...
	private void dumpPageDataLeaf(FileStore store, int pageSize, PrintWriter writer, DataPage s, boolean last, long pageId)
			throws SQLException {
		storageId = s.readInt();
