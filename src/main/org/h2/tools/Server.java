...
public class Server implements Runnable, ShutdownHandler {
    ...
    /**
     * The command line interface for this tool. The options must be split into strings like this: "-baseDir", "/temp/data",... By default,
     * -tcp, -web, -browser and -pg are started. If there is a problem starting a service, the program terminates with an exit code of 1.
     ...
     * <li>-browser (start a browser and open a page to connect to the Web Server)</li>
     * <li>-tcp (start the TCP Server)</li>
     * <li>-tcpShutdown {url} (shutdown the running TCP Server, URL example: tcp://localhost:9094)</li>
     * <li>-pg (start the PG Server)</li>
     * <li>-ftp (start the FTP Server)</li>
     * <li>-trace (print additional trace information; for all servers)</li>
     * <li>-baseDir {directory} (sets the base directory for H2 databases; for all servers)</li>
     ...
     * <li>-tcpPassword {password} (the password for shutting down a TCP Server)</li>
    ...
    /**
     * Shutdown a TCP server. If force is set to false, the server will not allow new connections, but not kill existing connections,
     * instead it will stop if the last connection is closed. If force is set to true, existing connections are killed. After calling the
     * method with force=false, it is not possible to call it again with force=true because new connections are not allowed. Example:
     * 
     * <pre>
     ...
     * <pre>
     * 
     * Server server = Server.createPgServer(new String[] { &quot;-pgAllowOthers&quot; }).start();
     * </pre>
     * 
     * @param args
     *            the argument list
    ...
    public Server start() throws SQLException {
        ...
        for (int i = 1; i < 64; i += i) {
            wait(i);
            if (isRunning(false)) { return this; }
        }
        if (isRunning(true)) { return this; }
    ...
    /**
     * Start a web server and a browser that uses the given connection. The current transaction is preserved. This is specially useful to
     * manually inspect the database when debugging.
     * 
     * @param conn
