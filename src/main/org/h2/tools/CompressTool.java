...
public class CompressTool {
    ...
    private byte[] getBuffer(int min) {
        ...
        if (min > MAX_BUFFER_SIZE) { return ByteUtils.newBytes(min); }
    ...
    private int readInt(byte[] buff, int pos) {
        ...
        int x = buff[pos++] & 0xff;
        if (x < 0x80) { return x; }
        if (x < 0xc0) { return ((x & 0x3f) << 8) + (buff[pos] & 0xff); }
        if (x < 0xe0) { return ((x & 0x1f) << 16) + ((buff[pos++] & 0xff) << 8) + (buff[pos] & 0xff); }
        if (x < 0xf0) { return ((x & 0xf) << 24) + ((buff[pos++] & 0xff) << 16) + ((buff[pos++] & 0xff) << 8) + (buff[pos] & 0xff); }
    ...
    public static OutputStream wrapOutputStream(OutputStream out, String compressionAlgorithm, String entryName) throws SQLException {
        ...
        try {
            ...
            else if (compressionAlgorithm != null) { throw Message.getSQLException(ErrorCode.UNSUPPORTED_COMPRESSION_ALGORITHM_1, compressionAlgorithm); }
            return out;
        }
        catch (IOException e) {
