...
public class ConditionInSelect extends Condition {
    ...
    public Value getValue(Session session) throws SQLException {
        ...
        if (!hasRow) { return ValueBoolean.get(false); }
    ...
    public Expression optimize(Session session) throws SQLException {
        ...
        query.prepare();
        if (query.getColumnCount() != 1) { throw Message.getSQLException(ErrorCode.SUBQUERY_IS_NOT_SINGLE_COLUMN); }
    ...
    public Expression optimizeInJoin(Session session, Select select) throws SQLException {
        ...
        if (all || compareType != Comparison.EQUAL) { return this; }
        if (!query.isEverything(ExpressionVisitor.EVALUATABLE)) { return this; }
