...
public class Function extends Expression implements FunctionCall {
    ...
    public static final int ABS = 0, ACOS = 1, ASIN = 2, ATAN = 3, ATAN2 = 4, BITAND = 5, BITOR = 6, BITXOR = 7, CEILING = 8, COS = 9, COT = 10, DEGREES = 11, EXP = 12, FLOOR = 13, LOG = 14, LOG10 = 15, MOD = 16, PI = 17, POWER = 18, RADIANS = 19, RAND = 20, ROUND = 21, ROUNDMAGIC = 22, SIGN = 23, SIN = 24, SQRT = 25, TAN = 26, TRUNCATE = 27, SECURE_RAND = 28, HASH = 29, ENCRYPT = 30, DECRYPT = 31, COMPRESS = 32, EXPAND = 33, ZERO = 34, RANDOM_UUID = 35;

    public static final int ASCII = 50, BIT_LENGTH = 51, CHAR = 52, CHAR_LENGTH = 53, CONCAT = 54, DIFFERENCE = 55, HEXTORAW = 56, INSERT = 57, INSTR = 58, LCASE = 59, LEFT = 60, LENGTH = 61, LOCATE = 62, LTRIM = 63, OCTET_LENGTH = 64, RAWTOHEX = 65, REPEAT = 66, REPLACE = 67, RIGHT = 68, RTRIM = 69, SOUNDEX = 70, SPACE = 71, SUBSTR = 72, SUBSTRING = 73, UCASE = 74, LOWER = 75, UPPER = 76, POSITION = 77, TRIM = 78, STRINGENCODE = 79, STRINGDECODE = 80, STRINGTOUTF8 = 81, UTF8TOSTRING = 82,
                    XMLATTR = 83, XMLNODE = 84, XMLCOMMENT = 85, XMLCDATA = 86, XMLSTARTDOC = 87, XMLTEXT = 88, REGEXP_REPLACE = 89, RPAD = 90, LPAD = 91;
    ...
    private static void addFunction(String name, int type, int parameterCount, int dataType, boolean nullIfParameterIsNull, boolean deterministic) {
    ...
    public void setParameter(int index, Expression param) throws SQLException {
        ...
        if (varArgs != null) {
            ...
            if (index >= args.length) { throw Message.getSQLException(ErrorCode.INVALID_PARAMETER_COUNT_2, new String[]{info.name, "" + args.length}); }
            args[index] = param;
        }
    }
    ...
    private Value getNullOrValue(Session session, Expression[] x, int i) throws SQLException {
        ...
        if (i < x.length) {
            Expression e = x[i];
            if (e != null) { return e.getValue(session); }
        }
        return null;
    }
    ...
    private Value getSimpleValue(Session session, Value v0, Expression[] args) throws SQLException {
        ...
        switch (info.type) {
            ...
            case COT: {
                ...
                if (d == 0.0) { throw Message.getSQLException(ErrorCode.DIVISION_BY_ZERO_1, getSQL()); }
    ...
    private Value getValueWithArgs(Session session, Expression[] args) throws SQLException {
        ...
        if (info.nullIfParameterIsNull) {
            for (int i = 0; i < args.length; i++) {
                if (getNullOrValue(session, args, i) == ValueNull.INSTANCE) { return ValueNull.INSTANCE; }
            }
        }
        Value v0 = getNullOrValue(session, args, 0);
        Value resultSimple = getSimpleValue(session, v0, args);
        if (resultSimple != null) { return resultSimple; }
        ...
        switch (info.type) {
            ...
            case MOD: {
                long x = v1.getLong();
                if (x == 0) { throw Message.getSQLException(ErrorCode.DIVISION_BY_ZERO_1, getSQL()); }
    ...
    private static int getDatePart(String part) throws SQLException {
        ...
        Integer p = (Integer) DATE_PART.get(StringUtils.toUpperEnglish(part));
        if (p == null) { throw Message.getSQLException(ErrorCode.INVALID_VALUE_2, new String[]{"date part", part}); }
        return p.intValue();
    }
    ...
    private static String insert(String s1, int start, int length, String s2) {
        ...
        int len2 = s2.length();
        start--;
        if (start < 0 || length <= 0 || len2 == 0 || start > len1) { return s1; }
    ...
    private static String hexToRaw(String s) throws SQLException {
        ...
        int len = s.length();
        if (len % 4 != 0) { throw Message.getSQLException(ErrorCode.DATA_CONVERSION_ERROR_1, s); }
    ...
    private double roundmagic(double d) {
        ...
        if ((d < 0.0000000000001) && (d > -0.0000000000001)) { return 0.0; }
        if ((d > 1000000000000.) || (d < -1000000000000.)) { return d; }
        StringBuilder s = new StringBuilder();
        s.append(d);
        if (s.toString().indexOf("E") >= 0) { return d; }
        int len = s.length();
        if (len < 16) { return d; }
        if (s.toString().indexOf(".") > len - 3) { return d; }
        s.delete(len - 2, len);
        len -= 2;
    ...
    protected void checkParameterCount(int len) throws SQLException {
        ...
        if (!ok) { throw Message.getSQLException(ErrorCode.INVALID_PARAMETER_COUNT_2, new String[]{info.name, min + ".." + max}); }
    }
    ...
    public void doneWithParameters() throws SQLException {
        ...
        if (info.parameterCount == VAR_ARGS) {
            ...
            int len = args.length;
            if (len > 0 && args[len - 1] == null) { throw Message.getSQLException(ErrorCode.INVALID_PARAMETER_COUNT_2, new String[]{info.name, "" + len}); }
        }
    }
    ...
    public Expression optimize(Session session) throws SQLException {
        ...
        switch (info.type) {
            ...
            case SET: {
                ...
                if (!(p0 instanceof Variable)) { throw Message.getSQLException(ErrorCode.CAN_ONLY_ASSIGN_TO_VARIABLE_1, p0.getSQL()); }
                break;
            }
            case FILE_READ: {
                if (args.length == 1) {
        ...
        switch (info.type) {
            ...
            case CONCAT:
                ...
                for (Expression arg : args) {
                    ...
                    displaySize = MathUtils.convertLongToInt((long) displaySize + arg.getDisplaySize());
                    if (precision < 0) {
    ...
    public ValueResultSet getValueForColumnList(Session session, Expression[] args) throws SQLException {
        ...
        switch (info.type) {
            case CSVREAD: {
                ...
                if (fileName == null) { throw Message.getSQLException(ErrorCode.PARAMETER_NOT_SET_1, "fileName"); }
    ...
    public boolean isEverything(ExpressionVisitor visitor) {
        ...
        if (visitor.getType() == ExpressionVisitor.DETERMINISTIC && !info.deterministic) { return false; }
        for (Expression arg : args) {
            Expression e = arg;
            if (e != null && !e.isEverything(visitor)) { return false; }
        }
        return true;
    }
