...
public class CompareLike extends Condition {
    ...
    public Expression optimize(Session session) throws SQLException {
        ...
        if (right.isValueSet() && (escape == null || escape.isValueSet())) {
            if (left.isValueSet()) { return ValueExpression.get(getValue(session)); }
            ...
            if (e == ValueNull.INSTANCE) { return ValueExpression.getNull(); }
    ...
    private char getEscapeChar(Value e) {
        ...
        if (e == null) { return Constants.DEFAULT_ESCAPE_CHAR; }
    ...
    public void createIndexConditions(Session session, TableFilter filter) throws SQLException {
        ...
        if (!right.isValueSet()) { return; }
        if (escape != null && !escape.isValueSet()) { return; }
    ...
    public Value getValue(Session session) throws SQLException {
        ...
        Value l = left.getValue(session);
        if (l == ValueNull.INSTANCE) { return l; }
        if (!isInit) {
            Value r = right.getValue(session);
            if (r == ValueNull.INSTANCE) { return r; }
            String p = r.getString();
            Value e = escape == null ? null : escape.getValue(session);
            if (e == ValueNull.INSTANCE) { return ValueNull.INSTANCE; }
            initPattern(p, getEscapeChar(e));
        }
    ...
    private boolean compareAt(String s, int pi, int si, int sLen) {
        ...
        for (; pi < patternLength; pi++) {
            ...
            switch (type) {
                case MATCH:
                    if ((si >= sLen) || !compare(s, pi, si++)) { return false; }
                    break;
                case ONE:
                    if (si++ >= sLen) { return false; }
                    break;
                case ANY:
                    if (++pi >= patternLength) { return true; }
                    while (si < sLen) {
                        if (compare(s, pi, si) && compareAt(s, pi, si, sLen)) { return true; }
                        si++;
                    }
                    return false;
                default:
    ...
    private void initPattern(String p, char escape) throws SQLException {
        ...
        for (int i = 0; i < len; i++) {
            ...
            int type;
            if (escape == c) {
                if (i >= len - 1) { throw Message.getSQLException(ErrorCode.LIKE_ESCAPE_ERROR_1, StringUtils.addAsterisk(p, i)); }
                c = p.charAt(++i);
                if (c != '_' && c != '%' && c != escape) { throw Message.getSQLException(ErrorCode.LIKE_ESCAPE_ERROR_1, StringUtils.addAsterisk(p, i)); }
                type = MATCH;
                lastAny = false;
            }
            else if (c == '%') {
