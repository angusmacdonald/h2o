...
public class Aggregate extends Expression {
    ...
    public Value getValue(Session session) throws SQLException {
        ...
        HashMap group = select.getCurrentGroup();
        if (group == null) { throw Message.getSQLException(ErrorCode.INVALID_USE_OF_AGGREGATE_FUNCTION_1, getSQL()); }
        AggregateData data = (AggregateData) group.get(this);
        if (data == null) {
            data = new AggregateData(type, dataType);
        }
        ...
        if (type == GROUP_CONCAT) {
            ...
            if (list == null || list.size() == 0) { return ValueNull.INSTANCE; }
            if (orderList != null) {
                try {
    ...
    public Expression optimize(Session session) throws SQLException {
        ...
        switch (type) {
            ...
            case SELECTIVITY:
                ...
                break;
            case SUM:
                if (!DataType.supportsAdd(dataType)) { throw Message.getSQLException(ErrorCode.SUM_OR_AVG_ON_WRONG_DATATYPE_1, getSQL()); }
    ...
    public String getSQL() {
        ...
        if (distinct) { return text + "(DISTINCT " + on.getSQL() + ")"; }
    ...
    public boolean isEverything(ExpressionVisitor visitor) {
        ...
        if (visitor.getType() == ExpressionVisitor.OPTIMIZABLE_MIN_MAX_COUNT_ALL) {
            switch (type) {
                ...
                case MIN:
                case MAX:
                    if (!SysProperties.OPTIMIZE_MIN_MAX) { return false; }
                    boolean first = type == MIN;
                    Index index = getColumnIndex(first);
                    return index != null;
                default:
        ...
        if (on != null && !on.isEverything(visitor)) { return false; }
        if (separator != null && !separator.isEverything(visitor)) { return false; }
        for (int i = 0; orderList != null && i < orderList.size(); i++) {
            SelectOrderBy o = (SelectOrderBy) orderList.get(i);
            if (!o.expression.isEverything(visitor)) { return false; }
        }
        return true;
    }
