...
public class Comparison extends Condition {
    ...
    /**
     * This is a pseudo comparison type that is only used for index conditions. It means the comparison will always yield FALSE. Example:
     * 1=0.
     */
    ...
    private Expression getCast(Expression expr, int dataType, long precision, int scale, int displaySize, Session session) throws SQLException {

        if (expr == ValueExpression.getNull()) { return expr; }
    ...
    public Expression optimize(Session session) throws SQLException {
        ...
        if (right == null) {
            ...
            if (lt == rt) {
                if (lt == Value.UNKNOWN) { throw Message.getSQLException(ErrorCode.UNKNOWN_DATA_TYPE_1, getSQL()); }
                dataType = lt;
            }
            else {
        ...
        if (compareType == IS_NULL || compareType == IS_NOT_NULL) {
            if (left.isConstant()) { return ValueExpression.get(getValue(session)); }
        }
        else {
            ...
            if (left.isConstant() && right.isConstant()) { return ValueExpression.get(getValue(session)); }
        }
        return this;
    }
    ...
    public Value getValue(Session session) throws SQLException {
        ...
        if (l == ValueNull.INSTANCE || r == ValueNull.INSTANCE) { return ValueNull.INSTANCE; }
     ...
     * @param compareType
     *            the compare type
     * @return the result of the comparison (1 if the first value is bigger, -1 if smaller, 0 if both are equal)
     */
    ...
    public void createIndexConditions(Session session, TableFilter filter) {
        ...
        if (l == null) {
            ...
            if (!left.isEverything(visitor)) { return; }
        }
        else if (r == null) {
            ExpressionVisitor visitor = ExpressionVisitor.get(ExpressionVisitor.NOT_FROM_RESOLVER);
            visitor.setResolver(filter);
            if (!right.isEverything(visitor)) { return; }
        }
        else {
    ...
    /**
     * Get an additional condition if possible. Example: given two conditions A=B and B=C, the new condition A=C is returned.
     * 
     * @param session
     *            the session
     * @param other
    ...
    Comparison getAdditional(Session session, Comparison other) {
        ...
        if (compareType == other.compareType && compareType == EQUAL) {
            ...
            else if (!(lc && l2c) && r.equals(r2)) { return new Comparison(session, EQUAL, left, other.left); }
        }
        return null;
    }
     ...
     * 
     * @param left
     *            true to get the left sub-expression, false to get the right sub-expression.
