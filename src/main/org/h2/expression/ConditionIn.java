...
public class ConditionIn extends Condition {
    ...
    public Value getValue(Session session) throws SQLException {
        ...
        if (values.size() == 0) { return ValueBoolean.get(false); }
        ...
        if (!result && hasNull) { return ValueNull.INSTANCE; }
    ...
    public Expression optimize(Session session) throws SQLException {
        ...
        if (values.size() == 0) { return ValueExpression.get(ValueBoolean.get(false)); }
        left = left.optimize(session);
        boolean constant = left.isConstant();
        if (constant && left == ValueExpression.getNull()) { return left; }
        ...
        if (constant && allValuesConstant) { return ValueExpression.get(getValue(session)); }
    ...
    public void createIndexConditions(Session session, TableFilter filter) {
        ...
        if (!SysProperties.OPTIMIZE_IN) { return; }
    ...
    public boolean isEverything(ExpressionVisitor visitor) {
        ...
        if (!left.isEverything(visitor)) { return false; }
    ...
    private boolean areAllValues(ExpressionVisitor visitor) {
        ...
        for (int i = 0; i < values.size(); i++) {
            Expression e = (Expression) values.get(i);
            if (!e.isEverything(visitor)) { return false; }
        }
        return true;
    }
    ...
    public Expression optimizeInJoin(Session session, Select select) throws SQLException {
        ...
        if (!areAllValues(ExpressionVisitor.get(ExpressionVisitor.EVALUATABLE))) { return this; }
        if (!(left instanceof ExpressionColumn)) { return this; }
