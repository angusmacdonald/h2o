...
/**
 * The constants defined in this class are initialized from system properties. Those properties can be set when starting the virtual
 * machine:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * They can be set within the application, but this must be done before loading any classes of this database (before loading the JDBC
 * driver):
 * 
 * <pre>
...
public class SysProperties {
     ...
     * When enabled, aliased columns (as in SELECT ID AS I FROM TEST) return the alias (I in this case) in ResultSetMetaData.getColumnName()
     * and 'null' in getTableName(). If disabled, the real column name (ID in this case) and table name is returned. This setting only
     * affects the default mode.
     * <p>
     * When using different modes, this feature is disabled for compatibility for all databases except MySQL. For MySQL, it is always
     * enabled.
     * </p>
     * <p>
     ...
     * The preferred browser to use. If not set, the default browser is used. For Windows, to use the Internet Explorer, set this property
     * to 'explorer'.
     */
     ...
     * How many time the cache size value is divided by two to get the index cache size. The index cache size is calculated like this:
     * cacheSize >> cacheSizeIndexShift.
     */
     ...
     * The minimum delay in milliseconds before an exception is thrown for using the wrong user name or password. This slows down brute
     * force attacks. The delay is reset to this value after a successful login. Unsuccessful logins will double the time until
     ...
     * The maximum delay in milliseconds before an exception is thrown for using the wrong user name or password. This slows down brute
     * force attacks. The delay is reset after a successful login. The value 0 means there is no maximum delay.
     */
     ...
     * Dollar quoting is used to quote text without having to use escape characters. A dollar quoted string starts and ends with $$. Inside
     * the text, $$ is not allowed.
     */
    ...
    /**
     * System property <code>h2.maxMemoryRowsDistinct</code> (default: Integer.MAX_VALUE).<br />
     * The maximum number of rows kept in-memory for SELECT DISTINCT queries. If more than this number of rows are in a result set, a
     ...
     * The minimum number of columns where a hash table is created when result set methods with column name (instead of column index)
     ...
     * Check the .lock.db file every this many milliseconds to detect that the database was changed. The process writing to the database
     * must first notify a change in the .lock.db file, then wait twice this many milliseconds before updating the database.
     */
     ...
     * This many bytes in main memory are allocated as a reserve. This reserve is freed up when if no memory is available, so that rolling
     * back a large transaction is easier.
     */
     ...
     * When true, ResultSet.getObject for CLOB or BLOB will return a java.sql.Clob / java.sql.Blob object. When set to false, it will return
     * a java.io.Reader / java.io.InputStream.
     * <p>
     * In version 1.0, it is disabled by default.
     * </p>
     * <p>
     ...
     * Linked connections should be shared, that means connections to the same database should be used for all linked tables that connect to
     * the same database.
     * <p>
     * In version 1.0, it is disabled by default.
     * </p>
    ...
    /**
     * INTERNAL. System property <code>h2.scriptDirectory</code> (default: empty string).<br />
