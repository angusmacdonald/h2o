...
public class ConstraintReferential extends Constraint {
	 ...
	 * @return the SQL statement
	 */
	public String getCreateSQLForCopy(Table table, Table refTable,
	 ...
	 *            the index
	 * @param isOwner
	 *            true if the index is generated by the system and belongs to
	 *            this constraint
	 */
	...
	private void checkRowOwnTable(Session session, Row oldRow, Row newRow)
		...
		for (IndexColumn column : columns) {
			int idx = column.column.getColumnId();
	...
	private boolean found(Session session, Index index, SearchRow check,
			Row excluding) throws SQLException {
	...
	public void setDeleteAction(int action) throws SQLException {
		...
		if (deleteAction != RESTRICT) {
			throw Message.getSQLException(
					ErrorCode.CONSTRAINT_ALREADY_EXISTS_1, "ON DELETE");
		}
	...
	public void setUpdateAction(int action) throws SQLException {
		...
		if (updateAction != RESTRICT) {
			throw Message.getSQLException(
					ErrorCode.CONSTRAINT_ALREADY_EXISTS_1, "ON UPDATE");
		}
		...
		if (action != CASCADE) {
			...
			for (int i = 0; i < columns.length; i++) {
				...
				if (action == SET_NULL) {
					...
					if (expr == null) {
						throw Message.getSQLException(
								ErrorCode.NO_DEFAULT_SET_1, column.getName());
					}
	...
	public boolean containsColumn(Column col) {
		...
		for (IndexColumn refColumn : refColumns) {
			if (refColumn.column == col) {
				return true;
			}
		}
		return false;
