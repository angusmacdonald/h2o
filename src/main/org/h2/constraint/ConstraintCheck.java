...
public class ConstraintCheck extends Constraint {
    ...
    public void checkRow(Session session, Table t, Row oldRow, Row newRow) throws SQLException {
        ...
        if (Boolean.FALSE.equals(expr.getValue(session).getBoolean())) { throw Message.getSQLException(ErrorCode.CHECK_CONSTRAINT_VIOLATED_1, getShortDescription()); }
    }
    ...
    public void checkExistingData(Session session) throws SQLException {
        ...
        if (r.next()) { throw Message.getSQLException(ErrorCode.CHECK_CONSTRAINT_VIOLATED_1, getName()); }
    }
