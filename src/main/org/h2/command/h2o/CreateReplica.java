...
public class CreateReplica extends SchemaCommand {
    ...
    public int update() throws SQLException, RemoteException {
        ...
        if (whereReplicaWillBeCreated != null || db.getFullDatabasePath().equals(whereReplicaWillBeCreated)) {
            int result = pushCommand(whereReplicaWillBeCreated, "CREATE REPLICA " + tableName + " FROM '" + whereDataWillBeTakenFrom + "'", true); // command
            // will
            // be
            // executed
            // elsewhere
        ...
        if (createEntirelyNewReplica) {
            ...
            if (pkColumns != null) {
                ...
                for (int i = 0; i < columns.size(); i++) {
                    ...
                    for (int j = 0; j < len; j++) {
                        if (pkColumns[j].columnName == null) pkColumns[j].columnName = pkColumns[j].column.getName();
        ...
        try {
            ...
            if (createEntirelyNewReplica) {
                ...
                if (!db.isTableLocal(getSchema())) {
                    ...
                    H2OEventBus.publish(new H2OEvent(this.session.getDatabase().getURL().getDbLocation(), DatabaseStates.REPLICA_CREATION, getSchema().getName() + "." + tableName));

                }
                else {
                    H2OEventBus.publish(new H2OEvent(this.session.getDatabase().getURL().getDbLocation(), DatabaseStates.META_TABLE_REPLICA_CREATION, getSchema().getName() + "." + tableName));
                }
            }
        }
        catch (SQLException e) {
    ...
    private void readMetaData() throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (currentColumns.contains(columnName)) // stops duplicate primary
            // keys - this happens
            // with multiple
            // replicas.
            continue;
