...
public abstract class Query extends Prepared {
	...
	/**
	 * The the list of select expressions. This may include invisible
	 * expressions such as order by expressions.
	 * 
	...
	/**
	 * Get the alias (or column name) of the first column. This is used to
	 * convert IN(SELECT ...) queries to inner joins.
	 * 
	 * @param session
	 *            the session
	...
	private boolean sameResultAsLast(Session session, Value[] params,
			Value[] lastParams, long lastEvaluated) throws SQLException {
		...
		if (!isEverything(ExpressionVisitor.DETERMINISTIC)
				|| !isEverything(ExpressionVisitor.INDEPENDENT)) {
			return false;
		}
		if (db.getModificationDataId() > lastEvaluated
				&& getMaxDataModificationId() > lastEvaluated) {
			return false;
		}
		return true;
	}
	 ...
	 */
	void initOrder(ObjectArray expressions, ObjectArray expressionSQL,
			ObjectArray orderList, int visible, boolean mustBeInResult)
			throws SQLException {
		for (int i = 0; i < orderList.size(); i++) {
			...
			if (e instanceof ExpressionColumn) {
				...
				String s = e.getSQL();
				for (int j = 0; expressionSQL != null
						&& j < expressionSQL.size(); j++) {
			...
			if (!isAlias) {
				if (mustBeInResult) {
					throw Message.getSQLException(
							ErrorCode.ORDER_BY_NOT_IN_RESULT, e.getSQL());
				}
				expressions.add(e);
				String sql = e.getSQL();
	 ...
	 */
	public SortOrder prepareOrder(ObjectArray orderList, int expressionCount)
			throws SQLException {
		...
		for (int i = 0; i < orderList.size(); i++) {
			...
			if (v == ValueNull.INSTANCE) {
				...
				if (idx < 0 || idx >= expressionCount) {
					throw Message.getSQLException(
							ErrorCode.ORDER_BY_NOT_IN_RESULT, "" + (idx + 1));
				}
			}
			index[i] = idx;
	...
	public final long getMaxDataModificationId() {
		ExpressionVisitor visitor = ExpressionVisitor
				.get(ExpressionVisitor.SET_MAX_DATA_MODIFICATION_ID);
		isEverything(visitor);
		return visitor.getMaxDataModificationId();
	}

	/**
	 * Visit all expressions and subqueries in this query using the visitor
	 * pattern.
	 * 
