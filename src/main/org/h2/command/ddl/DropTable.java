...
public class DropTable extends SchemaCommand {
	...
	private void executeDrop(String transactionName) throws SQLException, RemoteException {
		...
		if ( table != null ) {
			...
			H2OEventBus.publish(new H2OEvent(db.getURL().getDbLocation(), DatabaseStates.TABLE_DELETION, getSchema().getName() + "."
					+ tableName));
		}
		if ( next != null ) {
		...
		if ( isRegularTable() ) {
			...
			if ( queryProxy == null || !queryProxy.getLockGranted().equals(LockType.WRITE) ) {
				...
				if ( tableManager == null ) {
					...
					queryProxy = QueryProxy.getQueryProxyAndLock(tableManager, fullTableName, database, lockToRequest, session
							.getDatabase().getLocalDatabaseInstanceInWrapper(), false);
				}
			}
