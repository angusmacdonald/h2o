...
public class TableData extends Table implements RecordReader {
	...
	public TableData(Schema schema, String tableName, int id, ObjectArray columns, boolean persistent, boolean clustered, int headPos)
			throws SQLException {
	...
	private void checkRowCount(Session session, Index index, int offset) {
		if ( SysProperties.CHECK && !database.isMultiVersion() ) {
			...
			if ( rc != rowCount + offset ) {
				...
				Message.throwInternalError("rowCount expected " + ( rowCount + offset ) + " got " + rc + " " + getName() + "."
						+ index.getName());
			}
		}
	}
	...
	public Index addIndex(Session session, String indexName, int indexId, IndexColumn[] cols, IndexType indexType, int headPos,
		...
		if ( index.needRebuild() && rowCount > 0 ) {
			try {
				...
				while ( cursor.next() ) {
					database.setProgress(DatabaseEventListener.STATE_CREATE_INDEX, getName() + ":" + index.getName(),
							MathUtils.convertLongToInt(i++), MathUtils.convertLongToInt(total));
					Row row = cursor.get();
					// index.add(session, row);
					buffer.add(row);
	...
	private void traceLock(Session session, boolean exclusive, String s) {
		if ( traceLock.isDebugEnabled() ) {
			traceLock
					.debug(session.getId() + " " + ( exclusive ? "exclusive write lock" : "shared read lock" ) + " " + s + " " + getName());
		}
	}
