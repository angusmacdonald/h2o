...
public class DateTimeUtils {
    ...
    private static long getUniversalTime(Calendar source, java.util.Date x) throws SQLException {
        ...
        if (source == null) { throw Message.getInvalidValueException("calendar", null); }
    ...
    private static long getLocalTime(java.util.Date x, Calendar target) throws SQLException {
        ...
        if (target == null) { throw Message.getInvalidValueException("calendar", null); }
    ...
    /**
     * Parse a date, time or timestamp value. This method supports the format +/-year-month-day hour:minute:seconds.fractional and an
    ...
    public static java.util.Date parseDateTime(String original, int type, int errorCode) throws SQLException {
        ...
        try {
            ...
            if (type != Value.TIME) {
                ...
                if (s1 <= 0 || s2 <= s1) { throw Message.getSQLException(errorCode, new String[]{s, "format yyyy-mm-dd"}); }
            ...
            if (type == Value.TIME || (type == Value.TIMESTAMP && s1 >= 0)) {
                ...
                if (s1 <= 0 || s2 <= s1) { throw Message.getSQLException(errorCode, new String[]{s, "format hh:mm:ss"}); }
                ...
                if (s.endsWith("Z")) {
                    ...
                    if (timeZoneStart >= 0) {
                        ...
                        if (!tz.getID().equals(tzName)) { throw Message.getSQLException(errorCode, new String[]{s, tz.getID() + " <>" + tzName}); }
