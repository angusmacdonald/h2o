...
public abstract class Tool {
    ...
    /**
     * Read an argument and check if it is true (1), false (-1), or not (0). This method is used for compatibility with older versions only.
     * 
     * @param args
    ...
    public static int readArgBoolean(String[] args, int i) {
        ...
        if (!SysProperties.OLD_COMMAND_LINE_OPTIONS) { return 0; }
        if (i + 1 < args.length) {
            String a = args[++i];
            if ("true".equals(a)) {
                return 1;
            }
            else if ("false".equals(a)) { return -1; }
        }
        return 0;
    }
