...
public class ByteUtils {
    ...
    /**
     * Calculate the index of the first occurrence of the pattern in the byte array, starting with the given index. This methods returns -1
     * if the pattern has not been found, and the start position if the pattern is empty.
     * 
     * @param bytes
     *            the byte array
     * @param pattern
    ...
    public static byte[] convertStringToBytes(String s) throws SQLException {
        ...
        int len = s.length();
        if (len % 2 != 0) { throw Message.getSQLException(ErrorCode.HEX_STRING_ODD_1, s); }
        len /= 2;
    ...
    /**
     * Compare two byte arrays. This method will always loop over all bytes and doesn't use conditional operations in the loop to make sure
     * an attacker can not use a timing attack when trying out passwords.
     * 
     * @param test
     *            the first array
     * @param good
    ...
    /**
     * Compare the contents of two byte arrays. If the content or length of the first array is smaller than the second array, -1 is
     * returned. If the content or length of the second array is smaller than the first array, 1 is returned. If the contents and lengths
    ...
    /**
     * Create an array of bytes with the given size. If this is not possible because not enough memory is available, an OutOfMemoryError
     * with the requested size in the message is thrown.
     * 
     * @param len
