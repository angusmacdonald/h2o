...
public class StringUtils {
    ...
    /**
     * Convert a string to a SQL literal. Null is converted to NULL. The text is enclosed in single quotes. If there are any special
     * characters, the method STRINGDECODE is used.
     * 
     * @param s
     *            the text to convert.
    ...
    /**
     * Convert a string to the Java literal using the correct escape sequences. The literal is not enclosed in double quotes. The result can
     * be used in properties files or in Java source code.
     * 
     * @param s
     *            the text to convert
    ...
    public static String javaDecode(String s) throws SQLException {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (c == '"') {
                ...
                if (i >= s.length()) { throw getFormatException(s, s.length() - 1); }
                c = s.charAt(++i);
                switch (c) {
                    case 't':
                        buff.append('\t');
    ...
    /**
     * Convert a UTF-8 representation of a text to the text using the given offset and length.
     * 
     * @param bytes
     *            the UTF-8 representation
     * @param offset
    ...
    public static String quoteJavaStringArray(String[] array) {
        ...
        StringBuilder buff = new StringBuilder(5 * array.length);
    ...
    public static String quoteJavaIntArray(int[] array) {
        ...
        if (array == null) { return "null"; }
        StringBuilder buff = new StringBuilder(2 * array.length);
        buff.append("new int[]{");
        for (int i = 0; i < array.length; i++) {
            if (i > 0) {
                buff.append(", ");
    ...
    public static String urlDecode(String encoded) {
        ...
        for (int i = 0; i < encoded.length(); i++) {
            ...
            if (ch == '+') {
                ...
                if (SysProperties.CHECK && (ch > 127 || ch < ' ')) { throw new IllegalArgumentException("unexpected char " + (int) ch + " decoding " + encoded); }
                buff[j++] = (byte) ch;
            }
        }
    ...
    /**
     * Split a string into an array of strings using the given separator. A null string will result in a null array, and an empty string in
    ...
    /**
     * Combine an array of strings to one array using the given separator character. A backslash and the separator character and escaped
     * using a backslash.
     * 
     * @param list
     *            the string array
    ...
    public static String arrayCombine(String[] list, char separatorChar) {
        ...
        StringBuilder buff = new StringBuilder(5 * list.length);
    ...
    private static SimpleDateFormat getDateFormat(String format, String locale, String timeZone) throws SQLException {
        ...
        try {
            ...
            if (locale == null) {
                ...
                // ## Java 1.4 end ##
                /*
                 * ## Java 1.3 only begin ## Locale l = new Locale(locale, ""); ## Java 1.3 only end ##
                 */
    ...
    /**
     * Creates an XML attribute of the form name="value". A single space is prepended to the name, so that multiple attributes can be
     * concatenated.
     * 
     * @param name
     *            the attribute name
    ...
    /**
     * Escapes a comment. If the data contains '--', it is converted to '- -'. The data is indented with 4 spaces if it contains a newline
     * character.
     * 
     * @param data
     *            the comment text
    ...
    public static String xmlCData(String data) {
        ...
        if (data.indexOf("]]>") >= 0) { return xmlText(data); }
    ...
    public static String pad(String string, int n, String padding, boolean right) {
        ...
        else if (n == string.length()) { return string; }
        char paddingChar;
     ...
     * @param sp
     *            what to remove (only the first character is used) or null for a space
