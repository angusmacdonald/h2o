...
public class MathUtils {
    ...
    /**
     * Round the value up to the next block size. The block size must be a power of two. As an example, using the block size of 8, the
     * following rounding operations are done: 0 stays 0; values 1..8 results in 8, 9..16 results in 16, and so on.
     * 
     * @param x
    ...
    public static BigDecimal setScale(BigDecimal bd, int scale) throws SQLException {
        ...
        else if (scale < -Constants.BIG_DECIMAL_SCALE_MAX) { throw Message.getInvalidValueException("" + scale, "scale"); }
    ...
    /**
     * Convert a long value to an int value. Values larger than the biggest int value is converted to the biggest int value, and values
     * smaller than the smallest int value are converted to the smallest int value.
     * 
     * @param l
