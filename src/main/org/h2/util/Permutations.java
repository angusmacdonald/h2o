...
/**
 * A class to iterate over all permutations of an array. The algorithm is from Applied Combinatorics, by Alan Tucker as implemented in
...
public class Permutations {
	...
	/**
	 * Move the index forward a notch. The algorithm first finds the rightmost index that is less than its neighbor to the right. This is
	 * the dip point. The algorithm next finds the least element to the right of the dip that is greater than the dip. That element is
	 * switched with the dip. Finally, the list of elements to the right of the dip is reversed. For example, in a permutation of 5 items,
	 * the index may be {1, 2, 4, 3, 0}. The dip is 2 the rightmost element less than its neighbor on its right. The least element to the
	 * right of 2 that is greater than 2 is 3. These elements are swapped, yielding {1, 3, 4, 2, 0}, and the list right of the dip point is
