...
public class ClassUtils {
    ...
    public static Class loadUserClass(String className) throws SQLException {
        ...
        if (!ALLOW_ALL && !ALLOWED_CLASS_NAMES.contains(className)) {
            ...
            if (!allowed) { throw Message.getSQLException(ErrorCode.ACCESS_DENIED_TO_CLASS_1, className); }
        }
        try {
            return Class.forName(className);
        }
    ...
    public static boolean isVarArgs(Method m) {
        ...
        if ("1.5".compareTo(SysProperties.JAVA_SPECIFICATION_VERSION) > 0) { return false; }
        try {
