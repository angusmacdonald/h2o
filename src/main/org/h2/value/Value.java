...
public abstract class Value {
    ...
    /**
     * Check if the two values are equal. No data conversion is made; this method returns false if the other object is not of the same
     * class.
     * 
     * @param other
     *            the other value
    ...
    /**
     * Get the higher value order type of two value types. If values need to be converted to match the other operands value type, the value
     * with the lower order is converted to the value with the higher order.
     * 
     * @param t1
    ...
    public static int getHigherOrder(int t1, int t2) throws SQLException {
        ...
        if (t1 == t2) {
            if (t1 == Value.UNKNOWN) { throw Message.getSQLException(ErrorCode.UNKNOWN_DATA_TYPE_1, "?, ?"); }
            return t1;
        }
        int o1 = getOrder(t1);
        int o2 = getOrder(t2);
    ...
    /**
     * Check if a value is in the cache that is equal to this value. If yes, this value should be used to save memory. If the value is not
     * in the cache yet, it is added.
     * 
     * @param v
        ...
        switch (type) {
            ...
            case DECIMAL: {
                ...
                switch (getType()) {
                    ...
                    case DOUBLE: {
                        double d = getDouble();
                        if (Double.isInfinite(d) || Double.isNaN(d)) { throw Message.getSQLException(ErrorCode.DATA_CONVERSION_ERROR_1, "" + d); }
                        return ValueDecimal.get(new BigDecimal(d));
                    }
                    case FLOAT: {
                        float f = getFloat();
                        if (Float.isInfinite(f) || Float.isNaN(f)) { throw Message.getSQLException(ErrorCode.DATA_CONVERSION_ERROR_1, "" + f); }
    ...
    public final boolean compareEqual(Value v) throws SQLException {
        ...
        else if (v == ValueNull.INSTANCE) { return false; }
        if (getType() == v.getType()) { return equals(v); }
    ...
    public final int compareTo(Value v, CompareMode mode) throws SQLException {
        ...
        if (getType() == v.getType()) { return compareSecure(v, mode); }
        int t2 = Value.getHigherOrder(getType(), v.getType());
    ...
    private byte convertToByte(long x) throws SQLException {
        ...
        if (x > Byte.MAX_VALUE || x < Byte.MIN_VALUE) { throw Message.getSQLException(ErrorCode.NUMERIC_VALUE_OUT_OF_RANGE); }
        return (byte) x;
    }

    private short convertToShort(long x) throws SQLException {

        if (x > Short.MAX_VALUE || x < Short.MIN_VALUE) { throw Message.getSQLException(ErrorCode.NUMERIC_VALUE_OUT_OF_RANGE); }
        return (short) x;
    }

    private int convertToInt(long x) throws SQLException {

        if (x > Integer.MAX_VALUE || x < Integer.MIN_VALUE) { throw Message.getSQLException(ErrorCode.NUMERIC_VALUE_OUT_OF_RANGE); }
        return (int) x;
    }
    ...
    private long convertToLong(BigDecimal x) throws SQLException {
        ...
        if (x.compareTo(MAX_LONG_DECIMAL) > 0 || x.compareTo(Value.MIN_LONG_DECIMAL) < 0) { throw Message.getSQLException(ErrorCode.NUMERIC_VALUE_OUT_OF_RANGE); }
