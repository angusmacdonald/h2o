...
/**
 * Implementation of the BLOB and CLOB data types. Small objects are kept in memory and stored in the data page of the record.
 * 
 * Large objects are stored in their own files. When large objects are set in a prepared statement, they are first stored as 'temporary'
 * files. Later, when they are used in a record, and when the record is stored, the lob files are linked: the file is renamed using the file
 * format (tableId).(objectId). There is one exception: large variables are stored in the file (-1).(objectId).
 * 
 * When lobs are deleted, they are first renamed to a temp file, and if the delete operation is committed the file is deleted.
 * 
...
public class ValueLob extends Value {
    ...
    public String getString() {
        ...
        try {
            if (type == Value.CLOB) {
                if (small != null) { return StringUtils.utf8Decode(small); }
    ...
    public Object getObject() {
        ...
        if (type == Value.CLOB) { return getReader(); }
        try {
    ...
    public String getTraceSQL() {
        ...
        if (small != null && getPrecision() <= SysProperties.MAX_TRACE_DATA_LENGTH) { return getSQL(); }
    ...
    public static boolean existsLobFile(String prefix) throws SQLException {
        ...
        String[] list = FileUtils.listFiles(dir);
        for (String name : list) {
            if (name.startsWith(prefix + ".") && name.endsWith(Constants.SUFFIX_LOB_FILE)) { return true; }
        }
        return false;
    }
    ...
    public int getMemory() {
        ...
        if (small != null) { return small.length + 32; }
        return 128;
    }
