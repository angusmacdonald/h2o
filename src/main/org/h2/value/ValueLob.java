...
public class ValueLob extends Value {
	...
	// and a getpart function (to get it in pieces) and make sure a file is
	// created!
	...
	private static String getFileName(DataHandler handler, int tableId,
		...
		return handler.getDatabasePath() + "." + tableId + "." + objectId
	 ...
	 */
	public static ValueLob open(int type, DataHandler handler, int tableId,
			int objectId, long precision, boolean compression) {
		String fileName = getFileName(handler, tableId, objectId);
		return new ValueLob(type, handler, fileName, tableId, objectId, true,
	 ...
	 * @return the lob value
	 */
	public static ValueLob createClob(Reader in, long length,
	...
	private void createFromReader(char[] buff, int len, Reader in,
			long remaining, DataHandler handler) throws SQLException {
		try {
	...
	private String[] getFileList(DataHandler handler, String dir)
			throws SQLException {
		SmallLRUCache cache = handler.getLobFileListCache();
		String[] list;
		if (cache == null) {
	 ...
	 * @return the lob value
	 */
	public static ValueLob createBlob(InputStream in, long length,
			DataHandler handler) throws SQLException {
		try {
			long remaining = Long.MAX_VALUE;
	...
	private FileStoreOutputStream initLarge(DataHandler handler)
			throws SQLException {
		this.handler = handler;
		this.tableId = 0;
		this.linked = false;
		...
		synchronized (handler) {
			if (handler.getLobFilesInDirectories()) {
				...
				fileName = getFileNamePrefix(handler.getDatabasePath(),
						objectId) + Constants.SUFFIX_TEMP_FILE;
			} else {
		...
		FileStoreOutputStream out = new FileStoreOutputStream(tempFile,
				handler, compressionAlgorithm);
		return out;
	}
	...
	public String getString() {
		int len = precision > Integer.MAX_VALUE || precision == 0 ? Integer.MAX_VALUE
				: (int) precision;
		try {
			if (type == Value.CLOB) {
				if (small != null) {
			...
			throw Message.convertToInternal(Message.convertIOException(e,
					fileName));
	...
	public byte[] getBytesNoCopy() throws SQLException {
		...
		try {
			return IOUtils.readBytesAndClose(getInputStream(),
					Integer.MAX_VALUE);
		} catch (IOException e) {
	...
	public InputStream getInputStream() throws SQLException {
		...
		return new BufferedInputStream(new FileStoreInputStream(store, handler,
				compression, alwaysClose), Constants.IO_BUFFER_SIZE);
	}
	...
	public String getTraceSQL() {
		if (small != null
				&& getPrecision() <= SysProperties.MAX_TRACE_DATA_LENGTH) {
			return getSQL();
		}
	...
	public boolean equals(Object other) {
		try {
			return other instanceof ValueLob
	 ...
	 *            the data handler
	 */
	public void convertToFileIfRequired(DataHandler handler)
			throws SQLException {
		if (Constants.AUTO_CONVERT_LOB_TO_FILES && small != null
				&& small.length > handler.getMaxLengthInplaceLob()) {
			boolean compress = handler.getLobCompressionAlgorithm(type) != null;
			int len = getBufferSize(handler, compress, Long.MAX_VALUE);
			int tabId = tableId;
			if (type == Value.BLOB) {
				createFromStream(ByteUtils.newBytes(len), 0, getInputStream(),
						Long.MAX_VALUE, handler);
			} else {
	...
	public static void removeAllForTable(DataHandler handler, int tableId)
		...
		if (handler.getLobFilesInDirectories()) {
			...
			for (String name : list) {
				if (name.startsWith(prefix + "." + tableId + ".")
						&& name.endsWith(Constants.SUFFIX_LOB_FILE)) {
	...
	private static void removeAllForTable(DataHandler handler, String dir,
	...
	private void copyFile(DataHandler handler, String fileName, String live)
			throws SQLException {
		synchronized (handler.getLobSyncObject()) {
