...
public class ValueLong extends Value {
    ...
    public Value add(Value v) throws SQLException {
        ...
        if (SysProperties.OVERFLOW_EXCEPTIONS) {
            ...
            if (sv != so || sr == so || sv == 0 || so == 0) { return ValueLong.get(result); }
            throw getOverflow();
        }
    ...
    public Value negate() throws SQLException {
        ...
        if (SysProperties.OVERFLOW_EXCEPTIONS) {
            if (value == Long.MIN_VALUE) { throw getOverflow(); }
        }
    ...
    public Value subtract(Value v) throws SQLException {
        ...
        if (SysProperties.OVERFLOW_EXCEPTIONS) {
            ...
            if (sv == so || so == 0) { return ValueLong.get(value - other.value); }
    ...
    public Value multiply(Value v) throws SQLException {
        ...
        if (SysProperties.OVERFLOW_EXCEPTIONS) {
            long result = value * other.value;
            if (value == 0 || value == 1 || other.value == 0 || other.value == 1) { return ValueLong.get(result); }
            if (isInteger(value) && isInteger(other.value)) { return ValueLong.get(result); }
            ...
            if (br.compareTo(MIN) < 0 || br.compareTo(MAX) > 0) { throw getOverflow(); }
    ...
    public Value divide(Value v) throws SQLException {
        ...
        ValueLong other = (ValueLong) v;
        if (other.value == 0) { throw Message.getSQLException(ErrorCode.DIVISION_BY_ZERO_1, getSQL()); }
        return ValueLong.get(value / other.value);
    }
    ...
    public static ValueLong get(long i) {
        ...
        if (i >= 0 && i < STATIC_SIZE) { return STATIC_CACHE[(int) i]; }
