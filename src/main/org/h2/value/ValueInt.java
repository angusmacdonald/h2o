...
public class ValueInt extends Value {
    ...
    public Value add(Value v) throws SQLException {
        ...
        if (SysProperties.OVERFLOW_EXCEPTIONS) { return checkRange((long) value + (long) other.value); }
        return ValueInt.get(value + other.value);
    }

    private ValueInt checkRange(long value) throws SQLException {

        if (value < Integer.MIN_VALUE || value > Integer.MAX_VALUE) { throw Message.getSQLException(ErrorCode.OVERFLOW_FOR_TYPE_1, DataType.getDataType(Value.INT).name); }
    ...
    public Value negate() throws SQLException {
        ...
        if (SysProperties.OVERFLOW_EXCEPTIONS) { return checkRange(-(long) value); }
