...
public class TriggerObject extends SchemaObjectBase {
	...
	private synchronized void load(Session session) throws SQLException {
		...
		try {
			...
			throw Message.getSQLException(ErrorCode.ERROR_CREATING_TRIGGER_OBJECT_3,
					new String[] { getName(), triggerClassName, e.toString() }, e);
		}
	}
	...
	/**
	 * Call the trigger class if required. This method does nothing if the trigger is not defined for the given action. This method is
	 * called before or after any rows have been processed, once for each statement.
	 * 
	 * @param session
	 *            the session
	 * @param beforeAction
	...
	public void fire(Session session, boolean beforeAction) throws SQLException {
		...
		try {
			...
			throw Message.getSQLException(ErrorCode.ERROR_EXECUTING_TRIGGER_3, new String[] { getName(), triggerClassName, e.toString() },
					e);
		} finally {
	...
	/**
	 * Call the fire method of the user-defined trigger class if required. This method does nothing if the trigger is not defined for the
	 * given action. This method is called before or after a row is processed, possibly many times for each statement.
	 * 
	 * @param session
	 *            the session
	 * @param oldRow
