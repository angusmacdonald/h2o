...
class PageBtreeLeaf extends PageBtree {
    ...
    void read() throws SQLException {
        ...
        if (tableId != index.getId()) { throw Message.getSQLException(ErrorCode.FILE_CORRUPTED_1, "page:" + getPageId() + " expected table:" + index.getId() + "got:" + tableId); }
    ...
    boolean remove(SearchRow row) throws SQLException {
        ...
        if (index.compareRows(row, getRow(at)) != 0) { throw Message.getSQLException(ErrorCode.ROW_NOT_FOUND_WHEN_DELETING_1, index.getSQL() + ": " + row); }
    ...
    void find(PageBtreeCursor cursor, SearchRow first, boolean bigger) throws SQLException {
        ...
        if (i > entryCount) {
            if (parentPageId == Page.ROOT) { return; }
