...
class PageDataLeaf extends PageData {
    ...
    void read() throws SQLException {
        ...
        if (tableId != index.getId()) { throw Message.getSQLException(ErrorCode.FILE_CORRUPTED_1, "page:" + getPageId() + " expected table:" + index.getId() + " got:" + tableId + " type:" + type); }
    ...
    /**
     * Add a row if possible. If it is possible this method returns 0, otherwise the split point. It is always possible to add one row.
     * 
     * @param row
     *            the now to add
            ...
            if (firstOverflowPageId != 0) {
                ...
                do {
                    ...
                    if (record == null) {
                        ...
                        if (!(record instanceof PageDataLeafOverflow)) { throw Message.getInternalError("page:" + next + " " + record, null); }
    ...
    PageDataLeaf getNextPage() throws SQLException {
        ...
        if (parentPageId == Page.ROOT) { return null; }
    ...
    protected void remapChildren() throws SQLException {
        ...
        if (firstOverflowPageId == 0) { return; }
    ...
    boolean remove(int key) throws SQLException {
        ...
        int i = find(key);
        if (keys[i] != key) { throw Message.getSQLException(ErrorCode.ROW_NOT_FOUND_WHEN_DELETING_1, index.getSQL() + ": " + key); }
