 ...
 * <ul>
 * <li>There is one {@link BtreeHead} that points to the root page. The head always stays where it is.</li>
 * <li>There is a number of {@link BtreePage}s. Each page is either a {@link BtreeNode} or a {@link BtreeLeaf}.</li>
 * <li>A node page links to other leaf pages or to node pages. Leaf pages don't point to other pages (but may have a parent).</li>
 * <li>The uppermost page is the root page. If pages are added or deleted, the root page may change.</li>
 * </ul>
...
public class BtreeIndex extends BaseIndex implements RecordReader {
     ...
     *            the index type
     * @param headPos
     *            the position of the index header page, or Index.EMPTY_HEAD for a new index
     */
    public BtreeIndex(Session session, TableData table, int id, String indexName, IndexColumn[] columns, IndexType indexType, int headPos) throws SQLException {
    ...
    private Cursor find(Session session, SearchRow first, boolean bigger, SearchRow last) throws SQLException {
        ...
        if (SysProperties.CHECK && storage == null) { throw Message.getSQLException(ErrorCode.OBJECT_CLOSED); }
        BtreePage root = getRoot(session);
        if (first == null) {
    ...
    public Cursor findFirstOrLast(Session session, boolean first) throws SQLException {
        ...
        if (first) {
            ...
            while (cursor.next()) {
                ...
                if (v != ValueNull.INSTANCE) { return cursor; }
            }
            return cursor;
        }
        BtreePage root = getRoot(session);
