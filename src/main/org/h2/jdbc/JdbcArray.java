...
public class JdbcArray extends TraceObject implements Array {
    ...
    private void checkClosed() throws SQLException {
        ...
        conn.checkClosed();
        if (value == null) { throw Message.getSQLException(ErrorCode.OBJECT_CLOSED); }
    }
    ...
    private Object[] get(long index, int count) throws SQLException {
        ...
        Object[] array = get();
        if (count < 0 || count > array.length) { throw Message.getInvalidValueException("" + count, "count (1.." + array.length + ")"); }
        if (index < 1 || index > array.length) { throw Message.getInvalidValueException("" + index, "index (1.." + array.length + ")"); }
