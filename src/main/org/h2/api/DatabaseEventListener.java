...
/**
 * A class that implements this interface can get notified about exceptions and
 * other events. A database event listener can be registered when connecting to
 * a database. Example database URL:
...
public interface DatabaseEventListener extends EventListener {
	...
	/**
	 * This method is called just after creating the object. This is done when
	 * opening the database if the listener is specified in the database URL,
	 * but may be later if the listener is set at runtime with the SET SQL
	 * statement.
	 * 
	 * @param url
	 *            - the database URL
	 ...
	 * inform the user and wait for it to clean up disk space. Another strategy
	 * is to send an email to the administrator in this method and then throw a
	 * SQLException. The database should not be accessed from within this method
	 * (even to close it).
	 * 
	 * @param stillAvailable
	 *            the estimated space that is still available, in bytes (if
	 *            known)
	 * @throws SQLException
