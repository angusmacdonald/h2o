...
/**
 * The transaction log system is responsible for the write ahead log mechanism used in this database. A number of {@link LogFile} objects
 * are used (one for each file).
 */
public class LogSystem {
    ...
    /**
     * This method is called when a 'prepare commit' log entry is read when opening the database.
     * 
     * @param log
     *            the log file
     * @param sessionId
    ...
    /**
     * Remove a session from the session state map. This is done when opening the database, when all records of this session have been
    ...
    public void prepareCommit(Session session, String transaction) throws SQLException {
        ...
        if (database == null || readOnly) { return; }
        synchronized (database) {
            if (closed) { return; }
    ...
    public void checkpoint() throws SQLException {
        ...
        if (readOnly || database == null) { return; }
        synchronized (database) {
    ...
    /**
     * Add a redo log entry to the file. This method is called when re-applying the log entries (when opening a database).
     * 
     * @param storage
     *            the target storage
     * @param recordId
