...
public class LogFile {
    ...
    static LogFile openIfLogFile(LogSystem log, String fileNamePrefix, String fileName) throws SQLException {
        ...
        if (!fileName.endsWith(Constants.SUFFIX_LOG_FILE)) { return null; }
        if (!FileUtils.fileStartsWith(fileName, fileNamePrefix + ".")) { return null; }
     ...
     * 
     * @param undo
     *            true if the operation should be undone
     * @param readOnly
    ...
    private boolean redoOrUndo(boolean undo, boolean readOnly) throws SQLException {
        ...
        if (readOnly && type != 'S') { return true; }
        if (undo) {
    ...
    void redoAllGoEnd() throws SQLException {
        ...
        if (length <= FileStore.HEADER_LENGTH) { return; }
        try {
     ...
     * 
     * @param delete
     *            if the file should be deleted shortly afterwards
     */
     ...
     * 
     * @param dataFile
     *            if his summary is for the data file
     * @param summary
     *            the summary
     */
     ...
     *            the session
     * @param storageId
     *            the id of the storage that was truncated
     * @param recordId
     *            the record id
     * @param blockCount
