...
public class Constants {
    ...
    /**
     * If the rounding mode should be BigDecimal.ROUND_HALF_UP when rounding from a decimal value to a long value.
     */
    public static final boolean CONVERT_TO_LONG_ROUND = true;

    /**
     * The cost is calculated on rowcount + this offset, to avoid using the wrong or no index if the table contains no rows _currently_
    ...
    /**
     * The default port number of the HTTP server (for the H2 Console). This value is also in the documentation.
     */
    ...
    /**
     * The lock mode that means table level locking is used for reads and writes. If a table is locked, System.gc is called to close
    ...
    /**
     * If old text file headers should be supported. This setting can be removed in future versions (required for compatibility with version
     * 1.1.103).
     */
    ...
    /**
     * The product name. This value must stay like that, see http://opensource.atlassian.com/projects/hibernate/browse/HHH-2682 H2O -
    ...
    /**
     * True if a test is being run but this test doesn't require the state of the System Table to be replicated. This is done because the
     * mechanism by which System Tables are replicated requires creating multiple threads, and is unnecessary for most tests.
     */
    ...
    /**
     * Get the complete version number of this database, consisting of the major version, the minor version, the build id, and the build
     * date.
     * 
