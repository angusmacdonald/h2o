...
public class Mode {
	 ...
	 * and 'null' in getTableName(). If disabled, the real column name (ID in this case) and table name is returned.
	 */
	public boolean aliasColumnName;
	
	/**
	 * When inserting data, if a column is defined to be NOT NULL and NULL is inserted, then a 0 (or empty string, or the current timestamp
	 * for timestamp columns) value is used. Usually, this operation is not allowed and an exception is thrown.
	 */
	public boolean convertInsertNullToZero;
	
	/**
	 * When converting the scale of decimal data, the number is only converted if the new scale is smaller then current scale. Usually, the
	 * scale is converted and 0s are added if required.
	 */
	...
	/**
	 * Concatenation of a NULL with another value results in NULL. Usually, the NULL is treated as an empty string if only one of the
	 * operators is NULL, and NULL is only returned if both values are NULL.
	 */
	...
	/**
	 * When using unique indexes, multiple rows with NULL in all columns are allowed, however it is not allowed to have multiple rows with
