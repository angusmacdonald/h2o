...
public class FunctionAlias extends DbObjectBase {
    ...
    public FunctionAlias(Database db, int id, String name, String javaClassMethod, boolean force) throws SQLException {
        ...
        if (lastDot < 0) { throw Message.getSQLException(ErrorCode.SYNTAX_ERROR_1, javaClassMethod); }
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            if (m.getName().equals(methodName) || getMethodSignature(m).equals(methodName)) {
                ...
                for (int j = 0; j < list.size(); j++) {
                    ...
                    if (old.getParameterCount() == javaMethod.getParameterCount()) { throw Message.getSQLException(ErrorCode.METHODS_MUST_HAVE_DIFFERENT_PARAMETER_COUNTS_2, new String[]{old.toString(), javaMethod.toString()}); }
                }
                list.add(javaMethod);
            }
        }
        if (list.size() == 0) { throw Message.getSQLException(ErrorCode.PUBLIC_STATIC_JAVA_METHOD_NOT_FOUND_1, methodName + " (" + className + ")"); }
    ...
    public JavaMethod findJavaMethod(Expression[] args) throws SQLException {
        ...
        for (JavaMethod m : javaMethods) {
            ...
            if (count == parameterCount || (m.isVarArgs() && count <= parameterCount + 1)) { return m; }
        }
    ...
    /**
     * There may be multiple Java methods that match a function name. Each method must have a different number of parameters however. This
     * helper class represents one such method.
     */
    public static class JavaMethod implements Comparable {
        ...
        public Value getValue(Session session, Expression[] args, boolean columnList) throws SQLException {
                ...
                if (o == null) {
                    if (paramClass.isPrimitive()) {
                        if (columnList) {
                            ...
                            // may
                            // be null
                            // need to set to default value otherwise the
                            // function
            ...
            try {
                ...
                try {
                    ...
                    if (returnValue == null) { return ValueNull.INSTANCE; }
        ...
        public int compareTo(Object o) {
            ...
            if (varArgs != m.varArgs) { return varArgs ? 1 : -1; }
            if (paramCount != m.paramCount) { return paramCount - m.paramCount; }
            if (hasConnectionParam != m.hasConnectionParam) { return hasConnectionParam ? 1 : -1; }
            return id - m.id;
        }
