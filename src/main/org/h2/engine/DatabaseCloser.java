...
/**
 * This class is responsible to close a database if the application did not close a connection. A database closer object only exists if
 * there is no user connected to the database.
 */
public class DatabaseCloser extends Thread {
    ...
    public void reset() {
        ...
        if (threadGroup != null && threadGroup.activeCount() > 100) {
            // in JDK 1.4 and below, all Thread objects are added to the
            // ThreadGroup,
