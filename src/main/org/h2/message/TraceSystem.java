...
/**
 * The trace mechanism is the logging facility of this database. There is usually one trace system per database. It is called 'trace'
 * because the term 'log' is already used in the database domain and means 'transaction log'. It is possible to write after close was
 * called, but that means for each write the log file will be opened and closed again (which is slower).
 */
public class TraceSystem implements TraceWriter {
	...
	/**
	 * This trace level means all type of messages should be written, but instead of using the trace file the messages should be written to
	 * SLF4J.
	 */
	...
	private void logWritingError(Exception e) {
		...
		SQLException se = Message.getSQLException(ErrorCode.TRACE_FILE_ERROR_2, new String[] { fileName, e.toString() }, e);
	...
	/**
	 * Close the writers, and the files if required. It is still possible to write after closing, however after each write the file is
	 * closed again (slowing down tracing).
	 */
	public void close() {
		closeWriter();
		closed = true;
