...
public class PageLog {
    ...
    /**
     * Run the recovery process. There are two recovery stages: first only the undo steps are run (restoring the state before the last
     * checkpoint). In the second stage the committed operations are re-applied.
     * 
     * @param undo
    ...
    void logAddOrRemoveRow(Session session, int tableId, Row row, boolean add) throws SQLException {
        ...
        try {
            if (trace.isDebugEnabled()) {
                trace.debug("log " + (add ? "+" : "-") + " table:" + tableId + " row:" + row);
            }
