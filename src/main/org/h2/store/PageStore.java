...
/**
 * This class represents a file that is organized as a number of pages. The first page (page 0) contains the file header, which is never
 * modified once the database is created. The format is:
 * <ul>
 * <li>0-47: file header (3 time "-- H2 0.5/B -- \n")</li>
 * <li>48-51: database page size in bytes (512 - 32768, must be a power of 2)</li>
...
public class PageStore implements CacheWriter {
    ...
    private void readHeader() throws SQLException {
        ...
        if (length < PAGE_SIZE_MIN) { throw Message.getSQLException(ErrorCode.FILE_CORRUPTED_1, fileName); }
        ...
        if (readVersion != 0) { throw Message.getSQLException(ErrorCode.FILE_VERSION_ERROR_1, fileName); }
        if (writeVersion != 0) {
            try {
                file.close();
            }
    ...
    public void setPageSize(int size) throws SQLException {
        ...
        if (size < PAGE_SIZE_MIN || size > PAGE_SIZE_MAX) { throw Message.getSQLException(ErrorCode.FILE_CORRUPTED_1, fileName); }
        boolean good = false;
        int shift = 0;
        for (int i = 1; i <= size;) {
            if (size == i) {
        ...
        if (!good) { throw Message.getSQLException(ErrorCode.FILE_CORRUPTED_1, fileName); }
        pageSize = size;
        pageSizeShift = shift;
    }
     ...
     *            the record
     * @param logUndo
     *            if an undo entry need to be logged
     * @param old
     ...
     * 
     * @param atEnd
     *            if the allocated page must be at the end of the file
     * @return the page id
     */
    ...
    public void readPage(int pos, DataPage page) throws SQLException {
        ...
        if (pos >= pageCount) { throw Message.getSQLException(ErrorCode.FILE_CORRUPTED_1, pos + " of " + pageCount); }
    ...
    /**
     * Run one recovery stage. There are two recovery stages: first (undo is true) only the undo steps are run (restoring the state before
     * the last checkpoint). In the second stage (undo is false) the committed operations are re-applied.
     * 
     * @param undo
    ...
    /**
     * Get the session state for this session. A new object is created if there is no session state yet.
     * 
     * @param sessionId
     *            the session id
