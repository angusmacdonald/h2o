...
public class DataPage {
    ...
    public void writeValue(Value v) throws SQLException {
        ...
        if (SysProperties.CHECK2) {
            if (pos - start != getValueLen(v)) { throw Message.throwInternalError("value size error: got " + (pos - start) + " expected " + getValueLen(v)); }
        }
    }
    ...
    public int getValueLen(Value v) throws SQLException {
        ...
        if (v == ValueNull.INSTANCE) { return 1; }
        switch (v.getType()) {
            case Value.BOOLEAN:
            case Value.BYTE:
            case Value.SHORT:
    ...
    public Value readValue() throws SQLException {
        ...
        if (dataType == '-') { return ValueNull.INSTANCE; }
        dataType = dataType - 'a';
        switch (dataType) {
            case Value.BOOLEAN:
