...
/**
 * This class represents a file that is usually written to disk. The two main files are .data.db and .index.db. For each such file, a number
 * of {@link Storage} objects exists. The disk file is responsible for caching; each object contains a {@link Cache} object. Changes in the
 * file are logged in a {@link LogSystem} object. Reading and writing to the file is delegated to the {@link FileStore} class.
 * <p>
 * There are 'blocks' of 128 bytes (DiskFile.BLOCK_SIZE). Each objects own one or more pages; each page size is 64 blocks
 * (DiskFile.BLOCKS_PER_PAGE). That is 8 KB page size. However pages are not read or written as one unit; only individual objects (multiple
 * blocks at a time) are read or written.
 * <p>
 * Currently there are no in-place updates. Each row occupies one or multiple blocks. Rows can occupy multiple pages. Rows are always
 * contiguous (except LOBs, they are stored in their own files).
 */
public class DiskFile implements CacheWriter {
    ...
    public void initFromSummary(byte[] summary) {
        ...
        synchronized (database) {
            ...
            if (database.getRecovery() || (initAlreadyTried && (!dataFile || !SysProperties.CHECK))) { return; }
            ...
            try {
                ...
                if (b2 > fileBlockCount) {
                    getTrace().info("unexpected size " + b2 + " when initializing summary for " + fileName + " expected:" + fileBlockCount);
                    return;
                }
                stage++;
                if (init) {
                ...
                getTrace().error("error initializing summary for " + fileName + " size:" + summary.length + " stage:" + stage, e);
     ...
     */
    Record getRecordIfStored(Session session, int pos, RecordReader reader, int storageId) throws SQLException {

        synchronized (database) {
            try {
                int owner = getPageOwner(getPage(pos));
                if (owner != storageId) { return null; }
                go(pos);
                rowBuff.reset();
    ...
    Record getRecord(Session session, int pos, RecordReader reader, int storageId) throws SQLException {
        ...
        synchronized (database) {
            if (file == null) { throw Message.getSQLException(ErrorCode.SIMULATED_POWER_OFF); }
            ...
            int id = s.readInt();
            if (storageId != id) {
                Message.throwInternalError("File ID mismatch got=" + id + " expected=" + storageId + " pos=" + pos + " " + logChanges + " " + this + " blockCount:" + blockCount);
            }
            if (blockCount == 0) {
    ...
    /**
     * Called after a session deleted a row. This sets the last uncommitted delete id in the session. This is used to make sure empty space
     * is not re-used before the change is committed.
     * 
     * @param session
     *            the session
     */
    ...
    int getPageOwner(int page) {
        ...
        if (page * BLOCKS_PER_PAGE > fileBlockCount || page >= pageOwners.size()) { return FREE_PAGE; }
    ...
    public void setPageOwner(int page, int storageId) throws SQLException {
        ...
        if (old == storageId) { return; }
        if (SysProperties.CHECK && old >= 0 && storageId >= 0 && old != storageId) {
            for (int i = 0; i < BLOCKS_PER_PAGE; i++) {
                if (used.get(i + page * BLOCKS_PER_PAGE)) {
                    Message.throwInternalError("double allocation in file " + fileName + " page " + page + " blocks " + (BLOCKS_PER_PAGE * page) + "-" + (BLOCKS_PER_PAGE * (page + 1) - 1));
                }
            }
        }
        if (old >= 0) {
    ...
    public int copyDirect(int pos, OutputStream out) throws SQLException {
        ...
        synchronized (database) {
            try {
                ...
                if (pos >= fileBlockCount) { return -1; }
    ...
    /**
     * Remove a record from the cache and mark it as deleted. This writes the old data to the transaction log.
     * 
     * @param session
     *            the session
     * @param pos
    ...
    /**
     * Add a record to the file. The position of the record must already be set before. This method will write the change to the transaction
     * log and will update the cache.
     * 
     * @param session
     *            the session
     * @param record
