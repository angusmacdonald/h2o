...
/**
 * The file lock is used to lock a database so that only one process can write to it. It uses a cooperative locking protocol. Usually a
 * .lock.db file is used, but locking by creating a socket is supported as well.
 */
public class FileLock {
    ...
    private void lockSocket() throws SQLException {
        ...
        if (!fs.createNewFile(fileName)) {
            ...
            if (read != fs.getLastModified(fileName)) { throw getExceptionFatal("Concurrent update", null); }
            fs.delete(fileName);
            if (!fs.createNewFile(fileName)) { throw getExceptionFatal("Another process was faster", null); }
        }
        try {
