...
public class WriterThread extends Thread {
     ...
     * 
     * Thread objects are not garbage collected until they returned from the run() method (even if they where never started) so if the
     * connection was not closed, the database object cannot get reclaimed by the garbage collector if we use a hard reference.
     */
    ...
    private void flushIndexes(Database database) {
        ...
        if (lastIndexFlush + Constants.FLUSH_INDEX_DELAY > time) { return; }
    ...
    /**
     * Stop the thread. This method is called when closing the database. Old log files are deleted as well.
     */
    ...
    /**
     * Delete the following log file later on. If there is already a file to be deleted, that one will be deleted immediately.
     * 
     * @param fileName
