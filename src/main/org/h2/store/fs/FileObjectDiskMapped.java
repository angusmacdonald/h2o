...
public class FileObjectDiskMapped implements FileObject {
        ...
        if (mapped != null) {
            ...
            if (useSystemGc) {
                ...
                while (bufferWeakRef.get() != null) {
                    if (System.currentTimeMillis() - start > GC_TIMEOUT_MS) { throw new RuntimeException("Timeout (" + GC_TIMEOUT_MS + " ms) reached while trying to GC mapped buffer"); }
                    System.gc();
                    Thread.yield();
                }
            }
    ...
    private void reMap() throws IOException {
        ...
        if (file.length() > Integer.MAX_VALUE) { throw new RuntimeException("File over 2GB is not supported yet"); }
        int oldPos = 0;
        if (mapped != null) {
