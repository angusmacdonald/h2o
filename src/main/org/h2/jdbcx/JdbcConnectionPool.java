 ...
 * (http://h2database.com/html/license.html). Initial Developer: Christian d'Heureuse, www.source-code.biz This class is dual-licensed LGPL
 * and under the H2 License. This module is free software: you can redistribute it and/or modify it under the terms of the GNU Lesser
 ...
 * version. See http://www.gnu.org/licenses/lgpl.html This program is distributed in the hope that it will be useful, but WITHOUT ANY
...
/**
 * A simple standalone JDBC connection pool. It is based on the <a href="http://www.source-code.biz/snippets/java/8.htm">
 * MiniConnectionPoolManager written by Christian d'Heureuse (Java 1.5) </a>. It is used as follows:
 * 
 * <pre>
 * // init
 * import org.h2.jdbcx.*;
...
public class JdbcConnectionPool implements DataSource {
    ...
    /**
     * Sets the maximum time in seconds to wait for a free connection. The default timeout is 60 seconds.
     * 
     * @param seconds
     *            the maximum timeout
     */
    ...
    /**
     * Retrieves a connection from the connection pool. If <code>maxConnections</code> connections are already in use, the method waits
     * until a connection becomes available or <code>timeout</code> seconds elapsed. When the application is finished using the connection,
     * it must close it in order to return it to the pool. If no connection becomes available within the given timeout, an exception with
     * SQL state 08001 and vendor code 8001 is thrown.
     * 
    ...
    public Connection getConnection() throws SQLException {
        ...
        for (int i = 0;; i++) {
            synchronized (this) {
                if (activeConnections < maxConnections) { return getConnectionNow(); }
                if (i >= timeout) { throw new SQLException("Login timeout", "08001", 8001); }
                try {
                    wait(1000);
                }
    ...
    /**
     * This method usually puts the connection back into the pool. There are some exception: If the pool is disposed, the connection is
     * disposed as well. If the pool is full, the connection is closed.
     * 
     * @param pc
    ...
    /**
     * This event listener informs the connection pool that about closed and broken connections.
     */
    ...
    /**
     * Returns the number of active (open) connections of this pool. This is the number of <code>Connection</code> objects that have been
     * issued by getConnection() for which <code>Connection.close()</code> has not yet been called.
     * 
